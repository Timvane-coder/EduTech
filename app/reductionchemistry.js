...
setThemeColors() {
    ...
    // Fix: Ensure equation is always a string
    const cleanInput = equation && typeof equation === 'string' ? this.cleanChemicalExpression(equation) : '';
    ...
    for (const [type, config] of Object.entries(this.redoxTypes)) {
        for (const pattern of config.patterns) {
            // Fix: Check if cleanInput, species, or parameters match
            const inputStr = cleanInput || '';
            const speciesStr = species || '';
            const paramsStr = JSON.stringify(parameters);
            
            if (pattern.test(inputStr) || pattern.test(speciesStr) || pattern.test(paramsStr)) {
                return {
                    originalInput: equation || species || `${type} problem`,
                    cleanInput: cleanInput,
                    type: type,
                    species: speciesStr,
                    parameters: { ...parameters },
    ...
    // Default to oxidation state if formula and charge provided
    if (parameters.formula && parameters.overallCharge !== undefined) {
        return {
            originalInput: `Find oxidation state in ${parameters.formula}`,
            cleanInput: cleanInput,
    ...
    // If we still can't determine type, use problemType if provided in parameters
    if (parameters.problemType) {
        return {
            originalInput: equation || species || 'Redox problem',
            cleanInput: cleanInput,
            type: parameters.problemType,
            species: species || '',
            parameters: { ...parameters },
            context: { ...context },
            parsedAt: new Date().toISOString()
    ...
    throw new Error(`Unable to recognize redox problem type from: ${equation || species || 'unknown input'}`);
}

cleanChemicalExpression(expression) {
    // Fix: Handle null/undefined input
...
solveRedoxProblem(config) {
    // Fix: Ensure config has required properties
    const { 
        equation = '', 
        species = '', 
        parameters = {}, 
    ...
    try {
        ...
        console.error('Error in solveRedoxProblem:', error);
...
4. Check units and significant figures`;
    ...
    if (solution.oxidationStates) {
        keyValues.push({ 
            label: 'Oxidation States', 
            value: JSON.stringify(solution.oxidationStates) 
        });
    }
    
    if (solution.cellPotential !== undefined && solution.cellPotential !== null) {
        keyValues.push({ 
            label: 'Cell Potential', 
            value: solution.cellPotential + ' V' 
        });
    }
    
    if (solution.freeEnergy !== undefined && solution.freeEnergy !== null) {
        keyValues.push({ 
            label: 'Free Energy', 
            value: solution.freeEnergy + ' kJ/mol' 
        });
    }
    
    if (solution.electronsTransferred !== undefined && solution.electronsTransferred !== null) {
        keyValues.push({ 
            label: 'Electrons Transferred', 
            value: solution.electronsTransferred 
        });
    }
    
    if (solution.isSpontaneous !== undefined && solution.isSpontaneous !== null) {
        keyValues.push({ 
            label: 'Spontaneous?', 
            value: solution.isSpontaneous ? 'Yes' : 'No' 
        });
    }
    ...
    if (solution.formula) {
        keyValues.push({
            label: 'Formula',
            value: solution.formula
        });
    }

    if (solution.molarMass !== undefined && solution.molarMass !== null) {
        keyValues.push({
            label: 'Molar Mass',
            value: solution.molarMass + ' g/mol'
        });
    }
...
createAnalysisSectionRedox() {
    ...
    return {
        ...
        type: 'analysis',
        content: {
            problemType: this.currentSolution.problemType || 'Redox Problem',
    ...
    // Fix: Add null checks for solution.category
    ...
    const interpretations = {
        ...
        'identifying_redox': solution.isRedox ? 
            `This is a redox reaction. ${solution.reducingAgent || 'Reducing agent'} is oxidized (loses electrons) and ${solution.oxidizingAgent || 'Oxidizing agent'} is reduced (gains electrons).` :
            'This is NOT a redox reaction - no electron transfer occurs.',
        
        'half_reactions': `Balanced using half-reaction method. ${solution.electronsTransferred || 0} electrons transferred. Oxidation at anode, reduction at cathode.`,
        
        'electrochemistry_galvanic': solution.isSpontaneous ?
            `Spontaneous galvanic cell with E°cell = ${solution.cellPotential || 0} V. Can produce ${Math.abs(solution.freeEnergy || 0)} kJ/mol of electrical work.` :
            `Non-spontaneous with E°cell = ${solution.cellPotential || 0} V. Would require external energy to proceed.`,
        
        'electrochemistry_electrolytic': `Electrolysis produced ${solution.massProduced || 0} g of ${solution.substance || 'product'} using ${solution.charge || 0} C of charge over ${solution.time || 0} seconds.`,
        
        'cell_potential': `Standard cell potential is ${solution.E_cell || 0} V. Reaction is ${solution.spontaneity || 'unknown'}.`,
        
        'nernst_equation': `At non-standard conditions, E = ${solution.cellPotential || 0} V (compared to E° = ${solution.E_standard || 0} V). Concentration effects ${(solution.cellPotential || 0) < (solution.E_standard || 0) ? 'decrease' : 'increase'} cell potential.`,
        
        'free_energy_equilibrium': `ΔG° = ${solution.freeEnergy || 0} kJ/mol indicates ${(solution.freeEnergy || 0) < 0 ? 'spontaneous, product-favored' : 'non-spontaneous, reactant-favored'} reaction.`,
        
        'disproportionation': solution.isDisproportionation ?
            `${solution.element || 'Element'} undergoes disproportionation: simultaneously oxidized to ${solution.oxidizedTo || 'higher state'} and reduced to ${solution.reducedTo || 'lower state'}.` :
            'This is not a disproportionation reaction.',
        
        'redox_titration': `Titration determined analyte concentration: ${solution.analyte?.concentration || 'unknown'} M using ${solution.titrant?.volume || 0} mL of ${solution.titrant?.molarity || 0} M titrant.`,
        
        'general': 'Solution obtained using redox principles.'
    };

    return interpretations[category] || interpretations['general'];
}
