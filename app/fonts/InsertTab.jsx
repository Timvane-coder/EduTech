...
export default function InsertTab({ onAddVisualization, sessionId }) {
  ...
  const [view, setView] = useState('options') // 'options', 'categories', 'items', 'configure', 'existing', 'search', 'gallery'
  ...
  // Load gallery items (similar to GraphTab)
  const loadGalleryItems = async () => {
    if (!sessionId) return
    
    setIsLoadingGallery(true)
    try {
      console.log(`🖼️ Loading gallery items for type: ${viewType}`)
      ...
      console.log(`✅ Loaded ${items.length} gallery item(s)`)
  ...
  // Load individual item image (similar to GraphTab's loadGraphImage)
  const loadItemImage = async (item) => {
    if (loadedImages[item.id] || imageErrors[item.id]) {
      return
    }

    try {
      console.log(`🖼️ Loading image for ${item.type}: ${item.id}`)
      ...
      if (response.ok) {
        ...
        console.log(`✅ Image loaded for ${item.id}`)
      } else {
        const error = await response.json()
        console.error(`❌ Failed to load image for ${item.id}:`, error)
        setImageErrors(prev => ({
          ...prev,
          [item.id]: error.error || 'Failed to load'
        }))
      }
    } catch (error) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/workbook/visualizations/search?type=${activeCategory}&query=${encodeURIComponent(searchQuery)}`)
      const data = await response.json()
  ...
  const handleInsert = async () => {
    ...
    try {
      ...
      // Refresh gallery if in gallery view
      if (view === 'gallery') {
  ...
  const showGeometryInfo = async () => {
    ...
    try {
      ...
      const info = `
Geometry: ${geometries[index].replace(/_/g, ' ').toUpperCase()}
...
Examples: ${data.examples?.join(', ')}
      ...
      console.error('Failed to get geometry info:', error)
    ...
    <div className="config-section">
      ...
      <div className="form-group">
        ...
        <input
          ...
          onChange={(e) => handleConfigFieldChange('width', parseInt(e.target.value))}
          min="400"
          max="1600"
        />
      </div>
      ...
      <div className="form-group">
        ...
        <input
          ...
          onChange={(e) => handleConfigFieldChange('height', parseInt(e.target.value))}
          min="300"
          max="1200"
        />
      </div>
      ...
      {activeCategory !== 'charts' && (
        <div className="form-group checkbox">
          <label>
            <input
              type="checkbox"
              checked={configData.showLabels !== false}
              onChange={(e) => handleConfigFieldChange('showLabels', e.target.checked)}
            />
            Show Labels
          </label>
        </div>
      ...
      {activeCategory === 'charts' && (
        <button className="next-btn" onClick={() => setConfigStep('data')}>
          Next: Configure Data →
        </button>
      )}

      {activeCategory !== 'charts' && activeCategory !== 'anatomical' && (
        <button className="next-btn" onClick={() => setConfigStep('options')}>
    ...
    return (
      <div className="config-section">
        ...
        {selectedItem.dataRequired.map((field) => (
          <div key={field} className="form-group">
            ...
            <small className="field-hint">
              ...
              {field === 'series' && 'Data values, separate ranges with : (e.g., B1:B7:C1:C7)'}
              {field === 'values' && 'Numeric values for chart (e.g., B2:B8)'}
        ...
        <div className="button-group">
          <button className="back-btn" onClick={() => setConfigStep('basic')}>
            ← Back
          </button>
          <button className="insert-btn" onClick={handleInsert}>
            ✅ Insert Chart
          ...
          <div className="form-group">
            ...
            <div className="checkbox-group">
              <label>
                <input
                  type="checkbox"
                  checked={configData.show2D !== false}
                  onChange={(e) => handleConfigFieldChange('show2D', e.target.checked)}
                />
                Show 2D Structure
              </label>
              <label>
                <input
                  type="checkbox"
                  checked={configData.show3D !== false}
                  onChange={(e) => handleConfigFieldChange('show3D', e.target.checked)}
                />
                Show 3D Model
              </label>
              <label>
                <input
                  type="checkbox"
                  checked={configData.showAngles !== false}
                  onChange={(e) => handleConfigFieldChange('showAngles', e.target.checked)}
                />
                Show Bond Angles
              </label>
            </div>
          ...
          {configData.show3D !== false && (
            <>
              <div className="form-group">
                ...
                <input
                  type="number"
                  value={configData.rotationX || 20}
                  onChange={(e) => handleConfigFieldChange('rotationX', parseInt(e.target.value))}
                  min="0"
                  max="360"
                />
              </div>
              <div className="form-group">
                ...
                <input
                  type="number"
                  value={configData.rotationY || 30}
                  onChange={(e) => handleConfigFieldChange('rotationY', parseInt(e.target.value))}
                  min="0"
                  max="360"
                />
              </div>
      ...
      {activeCategory === 'anatomical' && selectedItem?.key === 'heartAnatomy' && (
        <>
          <div className="form-group">
            ...
            <select
              value={configData.chamber || 'wholeheart'}
              onChange={(e) => handleConfigFieldChange('chamber', e.target.value)}
            >
          ...
          <div className="form-group checkbox">
            <label>
              <input
                type="checkbox"
                checked={configData.showBloodFlow !== false}
                onChange={(e) => handleConfigFieldChange('showBloodFlow', e.target.checked)}
              />
              Show Blood Flow
            </label>
          </div>
    ...
    <div className="insert-tab-wrapper">
      <div className="scrollable-content">
        <div className="tab-header">
          ...
          <p className="subtitle">Create and view charts, diagrams, and molecular structures</p>
        </div>
        ...
        {view !== 'options' && (
          <div className="breadcrumb">
            ...
            {view === 'existing' && (
              <>
                <span>›</span>
                <span className="current">Existing Items</span>
              </>
            )}
            {view === 'gallery' && (
              <>
                <span>›</span>
                <span className="current">Image Gallery</span>
              </>
            )}
            {view === 'items' && (
              <>
        ...
        {/* Gallery View (merged from ViewTab) */}
        {view === 'gallery' && (
          <div className="gallery-view">
            ...
            {isLoadingGallery ? (
              ...
              <div className="empty-state">
                ...
                <h3>No Images Yet</h3>
                <p>Create charts or diagrams to see them here</p>
              </div>
            ) : (
                ...
                {galleryItems.map((item) => (
                  <div key={item.id} className="gallery-card">
                    ...
                    <div className="gallery-info">
                      ...
                      {item.formula && (
                        <div className="gallery-formula">{item.formula}</div>
                      )}
          ...
          <div className="search-results-view">
            ...
            {searchResults.length === 0 ? (
              <div className="empty-state">
                ...
                <button className="back-btn" onClick={resetToOptions}>
                  ← Back to Options
                </button>
              </div>
            ) : (
              <div className="items-grid">
                {searchResults.map((item, index) => (
                  ...
                  >
                    ...
                    {item.formula && (
                      <div className="item-formula">Formula: {item.formula}</div>
                    )}
                    {item.category && (
        ...
        {view === 'existing' && (
          <div className="existing-items-view">
            <h4 className="section-title">
              Existing {activeCategory} in Workbook ({existingItems.length})
            </h4>
                    ...
                    <div className="item-details">
                      ...
                      {item.formula && (
                        <div className="detail-row">
                          ...
                          <span className="detail-value">{item.formula}</span>
                        </div>
                      )}
                      <div className="detail-row">
              ...
              <div className="items-grid">
                {items.map((item, index) => (
                  ...
                  >
                    ...
                    {item.dataRequired && (
                      <div className="item-data-req">
                        Requires: {item.dataRequired.join(', ')}
                      </div>
                    )}
                  </div>
                ))}
        ...
        {view === 'configure' && (
          <div className="configure-view">
            ...
            <div className="config-steps">
              <div className={`step ${configStep === 'basic' ? 'active' : ''}`}>
                1. Basic Settings
              </div>
              {activeCategory === 'charts' && (
                <div className={`step ${configStep === 'data' ? 'active' : ''}`}>
                  2. Data Configuration
                </div>
              )}
              {(activeCategory !== 'charts' || selectedItem?.key === 'heartAnatomy') && (
                <div className={`step ${configStep === 'options' ? 'active' : ''}`}>
                  {activeCategory === 'charts' ? '3' : '2'}. Advanced Options
                </div>
              )}
            </div>
      ...
      <style jsx>{`
       ...
       .data-help {
        ...
        .form-group select {                                              width: 100%;
          ...
          border: 2px solid rgba(59, 130, 246, 0.3);                      border-radius: 6px;
          color: #e5e7eb;
          font-size: 14px;
