...
function generateRelatedPercentYield() {
    ...
    relatedProblems.push({
        ...
        problem: 'If theoretical yield is 75.0 g and % yield is 78.5%, find actual yield',
        parameters: { theoreticalYield: 75.0, percentYield: 78.5 },
        type: 'percent_yield',
        context: { difficulty: 'intermediate', topic: 'Yield Calculations' },
        subparts: [
            'Given: Theoretical = 75.0 g, % yield = 78.5%',
            'Rearrange formula: actual = theoretical × (% yield / 100)',
            'actual = 75.0 g × (78.5 / 100)',
            'actual = 75.0 g × 0.785',
            'actual = 58.9 g'
        ],
        helper: 'Multiply theoretical by decimal form of percentage',
        solution: '58.9 g',
        realWorldContext: 'Predicting actual product amounts'
    });

    // Problem 3: Complete Yield Problem
    relatedProblems.push({
        difficulty: 'harder',
        scenario: 'Complete Yield Problem',
        problem: '10.0 g Mg + excess O₂ → MgO. Obtained 14.8 g MgO. Find % yield',
        parameters: {
...
function generateRelatedGasStoichiometry() {
    ...
    relatedProblems.push({
        ...
        parameters: { moles: 2.0, useSTP: true },
    ...
    relatedProblems.push({
        ...
        parameters: { pressure: 2.0, volume: 10.0, temperature: 300, useSTP: false },
        type: 'gas_stoichiometry',
    ...
    // Problem 3: Gas Stoichiometry
    relatedProblems.push({
        difficulty: 'harder',
        scenario: 'Gas Stoichiometry Problem',
        problem: 'How many liters of O₂ (STP) needed to burn 5.0 L CH₄? CH₄ + 2O₂ → CO₂ + 2H₂O',
        parameters: {
...
function generateRelatedThermochemical() {
    ...
    // Problem 2: Heat and Stoichiometry
    relatedProblems.push({
        difficulty: 'harder',
        scenario: 'Heat and Product Formation',
        problem: 'If 2NH₃ → N₂ + 3H₂ absorbs 92 kJ, how much heat to produce 5.0 L H₂ at STP?',
        parameters: {
            equation: '2NH3 → N2 + 3H2, ΔH = +92 kJ',
            productVolume: 5.0,
            productFormula: 'H2',
            enthalpyChange: 92,
            useSTP: true
        ...
        type: 'thermochemical',
        context: { difficulty: 'advanced', topic: 'Thermochemical Stoichiometry' },
        subparts: [
            'Given: 5.0 L H₂ at STP, ΔH = +92 kJ for reaction as written',
            'Step 1: Find moles of H₂',
            'n(H₂) = 5.0 L / 22.4 L/mol = 0.223 mol',
            'Step 2: Reaction shows 3 mol H₂ per 92 kJ',
            'Heat per mol H₂ = 92 kJ / 3 mol = 30.7 kJ/mol',
            'Step 3: Calculate total heat',
            'Heat = 0.223 mol × 30.7 kJ/mol = 6.8 kJ',
            'Positive = endothermic (absorbed)'
        ],
        helper: 'Find moles first, then use enthalpy ratio',
        solution: '6.8 kJ absorbed',
        realWorldContext: 'Energy requirements in chemical processes'
    });

    // Problem 3: Hess's Law
    relatedProblems.push({
        difficulty: 'harder',
        ...
        parameters: {
            ...
            givenEquations: [
                { equation: 'C + 0.5O2 → CO', enthalpy: -110 },
                { equation: 'CO + 0.5O2 → CO2', enthalpy: -283 }
            ]
        },
        type: 'thermochemical',
        context: { difficulty: 'advanced', topic: 'Hess\'s Law' },
        subparts: [
            'Target: C + O₂ → CO₂',
            'Given equations:',
            '(1) C + ½O₂ → CO, ΔH₁ = -110 kJ',
            ...
            'C + O₂ → CO₂',
            'Add enthalpies: ΔH = ΔH₁ + ΔH₂',
            'ΔH = -110 kJ + (-283 kJ) = -393 kJ'
        ],
        helper: 'Add equations and their enthalpies',
        solution: 'ΔH = -393 kJ',
        realWorldContext: 'Indirect determination of enthalpy changes'
    });
...
function generateRelatedEmpiricalFormula() {
    ...
    relatedProblems.push({
        ...
        problem: 'Burning 0.500 g compound gives 1.467 g CO₂ and 0.600 g H₂O. Find empirical formula',
        parameters: {
            compoundMass: 0.500,
            CO2mass: 1.467,
            H2Omass: 0.600,
        ...
        type: 'empirical_formula',
        context: { difficulty: 'advanced', topic: 'Combustion Analysis' },
        subparts: [
            'Given: 0.500 g compound → 1.467 g CO₂, 0.600 g H₂O',
            'Step 1: Find moles of C from CO₂',
            'n(CO₂) = 1.467 / 44.009 = 0.0333 mol',
            'n(C) = 0.0333 mol (1:1 ratio)',
            'Step 2: Find moles of H from H₂O',
            'n(H₂O) = 0.600 / 18.015 = 0.0333 mol',
            'n(H) = 2 × 0.0333 = 0.0666 mol (2:1 ratio)',
            'Step 3: Find mole ratio',
            'C: 0.0333 / 0.0333 = 1',
            'H: 0.0666 / 0.0333 = 2',
            'Empirical formula: CH₂'
        ],
        helper: 'C from CO₂ (1:1), H from H₂O (2:1)',
        solution: 'CH₂',
        realWorldContext: 'Identifying unknown organic compounds'
    });
    ...
    relatedProblems.push({
        ...
        parameters: {
            percentComposition: { P: 26.7, N: 12.1, Cl: 61.2 }
        },
        type: 'empirical_formula',
