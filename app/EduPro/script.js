...
/*
 * ─── Mitosis Animation Integration ──────────────────────────────────
 * Replaces the house animation block entirely.
 * Handles: Design Now click → panel appears → MitosisAnim.start()
 *          Go Back click    → MitosisAnim.stop() → panel hides → main content
 */

// Preload organelle images as soon as DOM is ready
MitosisAnim.preload(function () {
  MitosisAnim.init();
});
...
// ── Show mitosis panel ──────────────────────────────────────────────
// ── Show mitosis panel ──────────────────────────────────────────────
function showMitosisPanel() {
  var panel  = document.getElementById('mitosis-panel');
  var logoId = '#flashingLogoDesign';
  var tx     = isPortrait() ? '46vw' : '26vw';
  var ty     = isPortrait() ? '25vh' : '32vh';
  var sc     = isPortrait() ? 10 : 7;

  // hide Design Now button
  gsap.to('#fl-design', {
    duration: 0.8,
    autoAlpha: 0,
    ease: 'slow(0.7, 0.1, false)',
  ...
  gsap.set('#hamburgerMenu', { display: 'none' });

  // logo expand animation into the panel
  gsap.to(logoId, {
    duration: 1.7,
    autoAlpha: 1,
    translateX: tx,
    ...
    onComplete: function () {
      ...
      // reset logo back to neutral so it's ready for next open
      gsap.set(logoId, {
        autoAlpha: 0,
        translateX: '0',
        translateY: '0',
      ...
      // clear GSAP inline styles left by hideMitosisPanel so
      // .mitosis-active CSS transition can take effect properly
      gsap.set(panel, { clearProps: 'opacity,visibility' });

      // activate the panel
      panel.classList.add('mitosis-active');

      // guard: MitosisBG module may still be loading on very first open
      if (!window.MitosisBG) {
        // fallback — just start cell animation without WebGPU bg
        MitosisAnim.start();
        return;
      }

      // start WebGPU parallax background first, then overlay cell animation
      MitosisBG.start().then(function () {
        MitosisAnim.start();
      });
    },
  });
...
// ── Hide mitosis panel ──────────────────────────────────────────────
function hideMitosisPanel() {

  // reverse cell animation back to frame 0
  MitosisAnim.reverse(function () {

    // brief pause so user sees the rewound start frame
    setTimeout(function () {

      // stop cell animation and clear both canvases
      MitosisAnim.stop(function () {

        var panel = document.getElementById('mitosis-panel');

        // fade out the entire mitosis panel
        gsap.to(panel, {
          duration: 0.6,
          autoAlpha: 0,
          onComplete: function () {

            // fully deactivate — remove class AND lock hidden state
            panel.classList.remove('mitosis-active');
            gsap.set(panel, {
              autoAlpha: 0,
              visibility: 'hidden',
            });

            // stop WebGPU background renderer to save resources
            if (window.MitosisBG) {
              MitosisBG.stop();
            }

            // restore the 3D scene panel underneath
            gsap.set('.threeD-panel', {
              autoAlpha: 1,
              translateY: '-200vh',
            });

            // restore nav
            gsap.set('#hamburgerMenu', { display: 'block' });
            gsap.to('#fl-design', { duration: 0.8, autoAlpha: 1 });

            // bring back main content titles and logo
            mainPgContentsAppear();
          },
        });
      });
...
// ── Click: Design Now ───────────────────────────────────────────────
$('#fl-design').on('click', function () {
  // guard against clicking before MitosisBG module has finished loading
  if (!window.MitosisBG) {
    console.warn('MitosisBG not ready yet');
    return;
  }
  mainPgContentsDisappear();
  setTimeout(showMitosisPanel, 600);
});

// ── Click: Mitosis Go Back ──────────────────────────────────────────
$('#mitosis-back-btn').on('click', function () {
  hideMitosisPanel();
});

// ── Sync Design Now visibility with main content page ───────────────
if (!window._mitosisWrapped) {
  window._mitosisWrapped = true;

  var _origAppear = mainPgContentsAppear;
  mainPgContentsAppear = function () {
    var anim = _origAppear();
    gsap.to('#fl-design', { delay: 1.5, duration: 1, autoAlpha: 1 });
    return anim;
  };

  var _origDisappear = mainPgContentsDisappear;
  mainPgContentsDisappear = function () {
    gsap.to('#fl-design', { duration: 0.3, autoAlpha: 0 });
    return _origDisappear();
  };
}
