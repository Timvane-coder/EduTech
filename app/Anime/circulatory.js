...
const CONFIG = {
    ...
    // Ghost / shadow floor — the dim anatomical map always visible beneath the animation.
    // Each value is the minimum opacity the element can reach (never goes to 0).
    ghost: {
        organ:      0.10,   // organs: heart, lungs, brain, etc.
        vessel:     0.08,   // arteries and veins
        capillary:  0.04,   // capillary beds (very subtle)
        valve:      0.06,   // heart valves
        heart:      0.14,   // heart gets a slightly stronger ghost (centrepiece)
    },
    circulatory: {
        centerX: 600,
        centerY: 400,
...
class BloodVessel {
    constructor(name, type, pathPoints, width) {
        ...
        this.opacity    = 0;           // scroll-driven brightening layer (0 → 1)
        this.ghostOpacity = (type === 'capillary')
            ? CONFIG.ghost.capillary
            : CONFIG.ghost.vessel;     // permanent shadow floor — never goes below this
    ...
    draw(renderer) {
        // Effective opacity: scroll-driven value, but never below the ghost floor
        const effectiveOpacity = Math.max(this.ghostOpacity, this.opacity);
        if (effectiveOpacity <= 0) return;

        const col  = this.getColor();
        col.a      = effectiveOpacity;
        const str  = this.getStroke();
        str.a      = effectiveOpacity;
        ...
        if (this.blockageFactor < 1 && this.pathPoints.length >= 2) {
            ...
            renderer.drawCircle(mid.x, mid.y, this.width * 0.6,
                `rgba(80,50,40,${effectiveOpacity * 0.85})`, null);
        }

        // Label — only show once scrolled in past ghost level
...
class Valve {
    constructor(name, position, radius = 7) {
        ...
        this.radius  = radius;
        this.opacity = 0;              // scroll-driven brightening
        this.ghostOpacity = CONFIG.ghost.valve;  // permanent shadow floor
        this.isOpen  = true;
    ...
    draw(renderer) {
        ...
        if (window.showLabels && this.opacity > 0.6) {
            renderer.drawText(
                ...
                `rgba(255,180,160,${this.opacity * 0.9})`, 'center'
            );
        }
    }
...
class Organ {
    constructor(name, type, shapeData, drawFn) {
        ...
        this.drawFn     = drawFn;
        this.opacity    = 0;              // scroll-driven brightening layer
        this.ghostOpacity = (type === 'heart')
            ? CONFIG.ghost.heart
            : CONFIG.ghost.organ;         // permanent shadow floor
        this.isHighlighted   = false;
        this.highlightPulse  = 0;
    ...
    draw(renderer) {
        // Always draw at least at ghost level — this creates the persistent shadow map
        const effectiveOpacity = Math.max(this.ghostOpacity, this.opacity);
        if (effectiveOpacity <= 0) return;

        // Temporarily set opacity to effectiveOpacity so drawFn uses it
        const savedOpacity = this.opacity;
        this.opacity = effectiveOpacity;
        ...
        if (this.isIschemic && this.shapeData.center) {
            ...
            renderer.drawGradientCircle(c.x, c.y, r * 1.2, [
                { offset: 0, color: `rgba(41,128,185,${effectiveOpacity * 0.35})` },
                { offset: 1, color: 'rgba(41,128,185,0)' }
            ]);
        }

        // Labels only appear once scrolled in past ghost level
...
class SceneManager {
    ...
    defineScenes() {
        return [
            // ── 0: Introduction ───────────────────────────
            {
                ...
                onUpdate: (p) => {
                    // All opacity at 0 — ghost floor handles the dim shadow map.
                    // As user scrolls into scene 1, the heart begins to brighten.
                    Object.values(this.system.organs).forEach(o  => { o.opacity = 0; });
                    Object.values(this.system.vessels).forEach(v => { v.opacity = 0; });
                    this.system.capillaries.forEach(c => { c.opacity = 0; });
                    Object.values(this.system.valves).forEach(v  => { v.opacity = 0; });
                }
            },
    ...
    fadeOrgan(name, p, startAt = 0) {
        ...
        if (!o) return;
        if (p < startAt) {
            // Scrolled back before this organ's reveal — drop to ghost floor
            o.opacity = 0;
            return;
        }
        const lp = (p - startAt) / (1 - startAt || 1);
        // opacity goes from 0 (ghost handles the floor) up to 1
...
function scrollFunction() {
    ...
    // Init app on very first scroll so the ghost shadow map is instantly visible
    if (scroll > 10 && !app) {
