...
export class EnhancedSkeletalSystemWorkbook {
    ...
    initializeSkeletalExperiments() {
        this.skeletalExperiments = {
            // ========================================
            // GALILEO'S BONE SCALING EXPERIMENT
            // ========================================
            
            galileo_bone_scaling: {
                name: "Galileo's Square-Cube Law and Bone Strength",
                relatedTopics: ['bone_structure', 'bone_development'],
                category: 'biomechanics',
                historicalBackground: {
                    scientist: "Galileo Galilei",
                    year: "1638",
                    work: "Dialogues Concerning Two New Sciences (Discorsi e Dimostrazioni Matematiche)",
                    discovery: "Square-Cube Law applied to biology",
                    observation: "Bones of larger animals are proportionally thicker than those of smaller animals",
                    principle: "When an object's size increases, its surface area increases by square of scale factor, but volume (and mass) increases by cube",
                    implication: "A simple scaled-up version of a small animal would collapse under its own weight",
                    quote: "From what has been demonstrated, you can plainly see the impossibility of increasing the size of structures to vast dimensions either in art or in nature... a small dog could probably carry on his back two or three dogs of his own size; but I believe that a horse could not carry even one of his own size.",
                    significance: "First mathematical treatment of biological scaling; founding principle of biomechanics",
                    context: "Galileo noticed that bones from large animals (elephants, horses) were proportionally much thicker than bones from small animals (mice, birds). He mathematically explained why giants and enormous creatures would be impossible.",
                    modernRelevance: "Explains why dinosaurs had massive bones, why insects can lift many times their weight but elephants cannot, and why there are limits to animal size"
                },
                theory: {
                    square_cube_law: {
                        statement: "When object doubles in size (all dimensions × 2):",
                        surface_area: "Increases by 2² = 4 times",
                        volume: "Increases by 2³ = 8 times",
                        mass: "Increases by 2³ = 8 times (if density constant)",
                        implication: "Weight increases faster than strength"
                    },
                    application_to_bone: {
                        bone_strength: "Proportional to cross-sectional area (A ∝ r²)",
                        body_weight: "Proportional to volume (W ∝ r³)",
                        scaling: "If animal doubles in height, strength increases 4×, but weight increases 8×",
                        consequence: "Larger animals need proportionally thicker bones",
                        stress: "Stress (force per area) = Weight / Cross-sectional area = (kr³) / (πr²) = kr",
                        conclusion: "Stress on bones increases linearly with size → larger animals have thicker bones relative to body size"
                    },
                    allometry: {
                        definition: "Study of how body proportions change with size",
                        isometric_scaling: "All dimensions scale equally (rare in nature)",
                        allometric_scaling: "Different body parts scale at different rates (common)",
                        bone_diameter: "Scales proportionally to body mass^(1/3) or length^(1.5) approximately",
                        example: "Elephant femur diameter is proportionally much greater than mouse femur diameter"
                    }
                },
                labExperiment: {
                    title: "Modeling Galileo's Square-Cube Law with Bone Analogs",
                    hypothesis: "As structural beams (bone analogs) increase in length while maintaining shape, their strength does not increase proportionally to their weight, leading to structural failure under self-weight if scaled beyond a critical size",
                    purpose: "Demonstrate square-cube law and understand why larger animals require proportionally thicker bones",
                    materials: [
                        "Wooden dowels or plastic rods of same diameter but different lengths (e.g., 10 cm, 20 cm, 30 cm, 40 cm)",
                        "Alternatively: 3 wooden dowels of same length but different diameters",
                        "Two support stands (e.g., stacks of books, clamps)",
                        "Weights (washers, coins, or calibrated masses)",
                        "Small container or bag to hold weights",
                        "String",
                        "Ruler or measuring tape",
                        "Calipers (for measuring diameter)",
                        "Calculator",
                        "Graph paper or computer for plotting",
                        "Optional: real bones (chicken, turkey) of different sizes for comparison"
                    ],
                    safetyPrecautions: [
                        "Be careful when handling weights",
                        "Dowels may break suddenly - wear safety glasses",
                        "Clear area below setup in case of falling weights"
                    ],
                    background: {
                        bone_as_beam: "Bone acts as beam supporting weight (body mass and external loads)",
                        failure: "Beam fails when stress exceeds material strength",
                        stress: "Force per unit area (σ = F/A)",
                        bending: "Cantilever beam or simply supported beam under load"
                    },
                    procedure: [
                        "SETUP:",
                        "Place two support stands (e.g., stacks of books) parallel, separated by distance slightly less than dowel length",
                        "Place shortest dowel (10 cm) across supports (simply supported beam)",
                        "Tie string to center of dowel",
                        "Attach weight container to string",
                        "",
                        "EXPERIMENT 1: Effect of Length (same diameter):",
                        "For each dowel length (10, 20, 30, 40 cm):",
                        "  Measure and record: length (L), diameter (d), calculate cross-sectional area A = π(d/2)²",
                        "  Place dowel across supports",
                        "  Gradually add weights to container until dowel breaks or deflects significantly (>5 cm)",
                        "  Record maximum load (breaking load) before failure",
                        "  Calculate stress at failure: σ = F/A",
                        "  Calculate weight of dowel itself (if significant)",
                        "  Observe: As length increases (scaling up), does strength increase proportionally?",
                        "",
                        "EXPERIMENT 2: Compensating with Diameter:",
                        "Using dowels of same length but different diameters (e.g., 5 mm, 10 mm, 15 mm):",
                        "  Repeat loading test",
                        "  Record breaking load for each diameter",
                        "  Calculate how diameter must increase to maintain strength when length increases",
                        "",
                        "OPTIONAL: Bone Comparison:",
                        "Obtain chicken or turkey bones of different sizes (e.g., drumstick from different age birds)",
                        "Measure bone length and diameter",
                        "Calculate diameter/length ratio",
                        "Compare to predictions from square-cube law"
                    ],
                    expectedResults: {
                        dowel_length: {
                            observation: "Longer dowels break under lighter loads (relative to cross-sectional area)",
                            explanation: "Bending moment increases with length; stress increases",
                            data: "Breaking load decreases as length increases (for same diameter)"
                        },
                        dowel_diameter: {
                            observation: "Thicker dowels support much more weight",
                            explanation: "Strength increases with cross-sectional area (∝ d²) and moment of inertia (∝ d⁴ for bending)",
                            data: "Breaking load increases rapidly with diameter"
                        },
                        scaling_calculation: {
                            simple_model: "If length doubles (L → 2L), diameter must increase by factor >√2 to maintain same stress",
                            actual: "For real beams under bending, diameter must increase by factor ~2^(3/2) ≈ 2.8"
                        }
                    },
                    dataTable: [
                        ["Length (cm)", "Diameter (mm)", "Area (mm²)", "Breaking Load (N)", "Stress (MPa)", "Load/Length"],
                        ["10", "5", "19.6", "50", "2.55", "5.0"],
                        ["20", "5", "19.6", "30", "1.53", "1.5"],
                        ["30", "5", "19.6", "20", "1.02", "0.67"],
                        ["40", "5", "19.6", "15", "0.77", "0.38"],
                        ["", "", "", "", "", ""],
                        ["Length (cm)", "Diameter (mm)", "Area (mm²)", "Breaking Load (N)", "Stress (MPa)", "Note"],
                        ["20", "5", "19.6", "30", "1.53", "Baseline"],
                        ["20", "7.5", "44.2", "100", "2.26", "1.5× diameter"],
                        ["20", "10", "78.5", "200", "2.55", "2× diameter"],
                    ],
                    calculations: {
                        cross_sectional_area: "A = π × (d/2)² = π × r²",
                        stress: "σ = Force / Area = F / A",
                        scaling: {
                            length_doubles: {
                                volume: "V ∝ L³ → 2³ = 8× volume",
                                mass: "m ∝ V → 8× mass",
                                weight: "W = mg → 8× weight",
                                cross_section: "A ∝ L² → 2² = 4× area (if isometric)",
                                stress: "σ = W/A → (8×)/(4×) = 2× stress",
                                conclusion: "Isometric scaling doubles stress → failure"
                            },
                            allometric_compensation: {
                                to_maintain_stress: "Need A ∝ W → A ∝ L³ → diameter ∝ L^(3/2)",
                                example: "L doubles → d must increase by 2^(3/2) = 2.83×"
                            }
                        }
                    },
                    analysis: {
                        square_cube_consequences: [
                            "Volume and mass increase as cube of linear dimension (L³)",
                            "Cross-sectional area increases as square (L²)",
                            "Therefore, stress (W/A) increases as L³/L² = L",
                            "Larger structures experience proportionally greater stress",
                            "To compensate, diameter must increase faster than length"
                        ],
                        biological_implications: [
                            "Small animals (mice): gracile bones relative to body size",
                            "Large animals (elephants): robust bones; diameter/length ratio much greater",
                            "Limits to size: Beyond certain size, bone would need to be so thick that organism couldn't function",
                            "Different strategies: Hollow bones (birds), different postures (elephants vs mice)",
                            "Metabolic limits also constrain size (surface/volume ratio affects heat loss)"
                        ],
                        real_bone_data: {
                            mouse_femur: "Length ~15 mm, diameter ~1.5 mm, ratio 0.10",
                            human_femur: "Length ~450 mm, diameter ~30 mm, ratio 0.067",
                            elephant_femur: "Length ~1000 mm, diameter ~120 mm, ratio 0.12",
                            observation: "Elephant has proportionally thicker bone than mouse, as Galileo predicted"
                        }
                    },
                    graphing: [
                        "Plot breaking load vs length (expect inverse relationship for same diameter)",
                        "Plot breaking load vs diameter (expect rapid increase, approximately ∝ d²)",
                        "Plot stress vs length (expect increase if diameter constant)",
                        "Plot diameter/length ratio vs total length for real bones (allometric scaling)"
                    ],
                    conclusions: [
                        "Galileo's square-cube law correctly predicts that larger structures cannot simply be scaled-up versions of smaller ones",
                        "Weight increases faster (L³) than structural strength (L²), requiring proportional thickening",
                        "Real animals compensate with allometric growth: larger animals have proportionally thicker bones",
                        "This principle explains limits to animal size and differences in skeletal proportions",
                        "Biomechanical principles constrain evolutionary possibilities"
                    ],
                    realWorldApplications: [
                        "Understanding why dinosaurs needed massive bones",
                        "Engineering structures (bridges, buildings scale similarly)",
                        "Why insects can lift many times their weight but elephants cannot",
                        "Limits to building height without new materials/designs",
                        "Prosthetic limb design (scaling from average to large/small individuals)"
                    ],
                    extensions: [
                        "Investigate hollow bones (birds) vs solid bones (mammals)",
                        "Compare bone strength in different materials (wood, plastic, metal, real bone)",
                        "Calculate theoretical maximum size of land animal given bone strength",
                        "Investigate aquatic animals (whales can be larger due to buoyancy)",
                        "Research allometric equations for different animal groups"
                    ],
                    historicalImpact: "Galileo's insight was revolutionary - it was the first time mathematical physics was applied to biology. It showed that biological form is constrained by physical laws, a principle that underpins modern biomechanics, comparative anatomy, and evolutionary biology."
                }
            },

            // ========================================
            // WOLFF'S LAW - BONE REMODELING UNDER STRESS
            // ========================================
            
            wolff_bone_remodeling: {
                name: "Wolff's Law: Bone Adaptation to Mechanical Stress",
                relatedTopics: ['bone_remodeling', 'bone_structure'],
                category: 'bone_physiology',
                historicalBackground: {
                    scientist: "Julius Wolff",
                    year: "1892",
                    work: "Das Gesetz der Transformation der Knochen (The Law of Bone Remodeling)",
                    discovery: "Wolff's Law: Bone adapts to the loads it experiences",
                    statement: "Every change in the form and function of bone, or of their function alone, is followed by certain definite changes in their internal architecture and equally definite secondary alteration in their external conformation, in accordance with mathematical laws.",
                    simplified: "Bone remodels in response to mechanical stress - strengthens where loaded, weakens where not",
                    observation: "Trabecular bone aligns along stress lines; bone density increases with exercise",
                    mechanism: "Osteocytes sense mechanical strain → signal osteoblasts (build) and osteoclasts (resorb)",
                    significance: "Explains how bone maintains optimal strength-to-weight ratio; basis for exercise prescriptions to prevent osteoporosis",
                    context: "Wolff noticed that femoral head trabeculae perfectly aligned with stress trajectories (visible in cross-section). He realized bone is not static but continuously optimizes its structure.",
                    modernUnderstanding: "Mechanotransduction - osteocytes detect fluid flow in canaliculi caused by loading, trigger biochemical signals",
                    clinical: "Bed rest → bone loss; weight-bearing exercise → bone gain; astronauts lose 1-2% bone mass per month in microgravity"
                },
                theory: {
                    mechanotransduction: {
                        stimulus: "Mechanical loading (compression, tension, shear)",
                        sensor: "Osteocytes in lacunae; most abundant bone cells",
                        mechanism: [
                            "Loading deforms bone matrix slightly",
                            "Deformation creates fluid flow in canaliculi",
                            "Osteocyte processes detect flow via mechanoreceptors (integrins, ion channels)",
                            "Osteocytes release signaling molecules (sclerostin, RANKL, OPG, prostaglandins, NO)",
                            "Signals recruit osteoblasts (formation) or osteoclasts (resorption)",
                            "Bone remodels to optimize structure for applied loads"
                        ],
                        response_time: "Weeks to months for significant remodeling"
                    },
                    types_of_loading: {
                        compression: "Squeezing force (e.g., standing, jumping)",
                        tension: "Stretching force (e.g., muscle pull on bone)",
                        shear: "Sliding force (parallel to surface)",
                        torsion: "Twisting force",
                        bending: "Combination of compression and tension on opposite sides"
                    },
                    strain: {
                        definition: "Deformation per unit length (ΔL/L); dimensionless",
                        physiologic_range: "~0.0004 (400 microstrain) to 0.003 (3000 microstrain)",
                        threshold: "Below ~400 microstrain → bone resorption; above → formation",
                        optimal: "~2000-2500 microstrain for maximal adaptation"
                    },
                    trabecular_alignment: {
                        principle: "Trabeculae orient along principal stress directions",
                        femoral_head: "Medial and lateral trabecular systems align with compression from body weight",
                        calcaneus: "Trabeculae oriented vertically (heel strike) and anteriorly (Achilles pull)",
                        efficiency: "Maximum strength with minimum material"
                    }
                },
                labExperiment: {
                    title: "Demonstrating Bone Adaptation Principles with Models and Data Analysis",
                    hypothesis: "Mechanical loading patterns determine bone structure; regions experiencing high stress develop increased density and aligned trabecular architecture",
                    purpose: "Visualize Wolff's Law through observation of real bones, stress models, and analysis of bone density changes with exercise",
                    variables: {
                        independent: "Type and magnitude of mechanical loading",
                        dependent: "Bone density, trabecular orientation, bone thickness",
                        controlled: "Bone type, age, species (for comparisons)"
                    },
                    materials: [
                        "Real bones (chicken, turkey, beef) - femur, humerus, vertebra recommended",
                        "Saw (to cut bone in half longitudinally)",
                        "Magnifying glass or dissecting microscope",
                        "Camera (to photograph trabecular patterns)",
                        "Graph paper",
                        "Optional: DEXA scan data or bone density measurements from literature",
                        "Optional: Finite element analysis software (free: FEBio, SimVascular) or simple beam calculator",
                        "Optional: Foam materials of different densities to model bone remodeling"
                    ],
                    safetyPrecautions: [
                        "Use saw carefully - adult supervision required",
                        "Wash hands thoroughly after handling raw bones",
                        "Sanitize work area",
                        "Wear gloves if desired"
                    ],
                    procedure: [
                        "PART 1: Observing Trabecular Architecture in Real Bones",
                        "",
                        "Obtain bones from butcher or leftover from meal (chicken leg, beef femur)",
                        "Carefully cut bone in half longitudinally (along length) with saw",
                        "  For femur: cut through femoral head, neck, shaft, and distal end",
                        "  For vertebra: cut sagittally (left-right)",
                        "Clean out marrow gently with water",
                        "Observe and photograph internal structure:",
                        "  Note compact bone thickness at different locations",
                        "  Observe trabecular (spongy) bone patterns in epiphyses",
                        "  Look for alignment of trabeculae - are they random or oriented?",
                        "Compare regions of high stress vs low stress",
                        "",
                        "FEMORAL HEAD ANALYSIS:",
                        "Identify two trabecular systems in femoral head (if using large mammal bone):",
                        "  Medial system: arches from medial cortex upward into head",
                        "  Lateral system: vertical struts from lateral cortex",
                        "  These align with compression stress lines from body weight",
                        "Sketch trabecular pattern on graph paper",
                        "Compare to engineering stress diagrams (look up 'femoral head stress distribution')",
                        "",
                        "VERTEBRAL BODY:",
                        "Observe vertical and horizontal trabeculae",
                        "Vertical trabeculae resist compression from body weight",
                        "Horizontal trabeculae prevent buckling",
                        "Note density difference: denser in central region (higher stress)",
                        "",
                        "PART 2: Simulating Stress and Remodeling",
                        "",
                        "Use online beam calculator or FEA software:",
                        "Model femur as cantilever beam (fixed at one end)",
                        "Apply load (body weight) and observe stress distribution",
                        "Note where stress is highest (predicts where bone should be thickest)",
                        "Compare to actual bone thickness measurements",
                        "",
                        "PART 3: Analyzing Exercise Effects (Data Analysis)",
                        "",
                        "Research studies on bone density and exercise:",
                        "Example: Tennis players - dominant arm vs non-dominant arm bone density",
                        "Example: Astronauts - bone loss in microgravity",
                        "Example: Bed rest studies - bone loss over time",
                        "Plot data from literature (if available):",
                        "  Bone density vs weeks of exercise",
                        "  Bone density vs weeks of bed rest",
                        "  Bone density in athletes vs sedentary controls",
                        "",
                        "PART 4 (Optional): Foam Model of Remodeling",
                        "",
                        "Use foam blocks of different densities to represent bone",
                        "Compress foam repeatedly in one direction",
                        "Observe: does foam 'adapt' (plastically deform) in direction of stress?",
                        "This crudely models bone remodeling (though bone is much more dynamic)"
                    ],
                    expectedObservations: {
                        femoral_head: {
                            trabecular_systems: "Two distinct systems (medial and lateral) oriented along stress lines",
                            compact_bone: "Thicker on inferior neck (high tensile stress); thinner on superior (lower stress)",
                            alignment: "Trabeculae NOT random - clearly oriented along predictable directions"
                        },
                        vertebral_body: {
                            pattern: "Vertical trabeculae (resist compression); horizontal (resist buckling)",
                            density: "Denser centrally where stress highest; sparser peripherally"
                        },
                        long_bone_shaft: {
                            compact_bone: "Thick cortex in shaft; thin in epiphyses",
                            hollow: "Medullary cavity reduces weight without sacrificing strength (most stress at outer surface in bending)"
                        },
                        comparison: {
                            high_stress_regions: "Dense trabecular bone, thick compact bone",
                            low_stress_regions: "Sparse trabeculae, thin compact bone or absent"
                        }
                    },
                    dataAnalysis: {
                        literature_examples: {
                            tennis_players: {
                                study: "Dominant arm bone density 10-15% higher than non-dominant",
                                interpretation: "Mechanical loading from hitting increases bone formation"
                            },
                            astronauts: {
                                study: "Lose 1-2% bone mass per month in microgravity (spine, hips)",
                                interpretation: "Lack of gravitational loading → bone resorption exceeds formation"
                            },
                            bed_rest: {
                                study: "Healthy adults lose ~1% bone density after 30 days strict bed rest",
                                interpretation: "Disuse osteopenia - bone adapts to reduced loading"
                            },
                            weight_bearing_exercise: {
                                study: "Resistance training increases bone density 1-3% over months",
                                interpretation: "Loading above physiologic threshold stimulates osteoblasts"
                            }
                        },
                        key_findings: [
                            "Bone density is plastic - increases with loading, decreases with disuse",
                            "Response is site-specific (loaded bones change, unloaded don't)",
                            "Takes weeks to months for measurable change",
                            "Effect persists as long as loading continues; reverses if stopped"
                        ]
                    },
                    modernTechniques: {
                        DEXA_scan: "Dual-energy X-ray absorptiometry - measures bone mineral density",
                        micro_CT: "High-resolution CT scanning - visualizes trabecular architecture in 3D",
                        finite_element_analysis: "Computer simulation of stress distribution in bones",
                        strain_gauges: "Directly measure bone strain in living animals during activity",
                        histomorphometry: "Microscopic analysis of bone formation/resorption rates"
                    },
                    calculations: {
                        stress: "σ = Force / Area",
                        strain: "ε = ΔLength / Original Length",
                        youngs_modulus: "E = σ / ε (stiffness of material)",
                        bone_strength: "Depends on: material properties (E, σ_max) and geometry (area, moment of inertia)"
                    },
                    molecularMechanisms: {
                        osteocyte_signaling: {
                            loading: "Compression → fluid flow in canaliculi → osteocyte mechanoreceptors activated",
                            anabolic_signals: "Osteocytes release bone morphogenetic proteins (BMPs), Wnt, IGF-1 → stimulate osteoblasts",
                            catabolic_signals: "Osteocytes reduce sclerostin (inhibitor of Wnt) → allows osteoblast activity",
                            RANKL_OPG: "Ratio of RANKL (stimulates osteoclasts) to OPG (inhibits osteoclasts) regulates resorption"
                        },
                        osteoblast_activation: {
                            mechanical_strain: "Above threshold → osteoblasts recruited to surface",
                            bone_formation: "Secrete osteoid → mineralization → new bone added",
                            response_time: "Weeks for new bone formation"
                        },
                        osteoclast_regulation: {
                            disuse: "Low strain → osteoclasts activated",
                            bone_resorption: "Remove bone from unloaded areas",
                            efficiency: "Reduces weight in regions where bone is unnecessary"
                        }
                    },
                    conclusions: [
                        "Bone structure is not predetermined but adapts dynamically to mechanical environment",
                        "Trabecular bone aligns along principal stress directions (visible in real bones)",
                        "Bone density increases with exercise, decreases with disuse (supported by human studies)",
                        "Wolff's Law explains bone optimization: maximum strength with minimum weight",
                        "Osteocytes are mechanosensors that orchestrate remodeling in response to strain"
                    ],
                    realWorldApplications: [
                        "Osteoporosis prevention: Weight-bearing exercise prescribed to maintain bone density",
                        "Fracture healing: Controlled loading accelerates healing (too little or too much impairs)",
                        "Spaceflight: Exercise protocols for astronauts to minimize bone loss",
                        "Prosthetics: Bone remodels around implants based on stress distribution ('stress shielding' problem)",
                        "Orthodontics: Teeth move through bone remodeling in response to applied forces",
                        "Evolutionary biology: Bone shape reflects habitual loading patterns (can infer behavior from fossils)"
                    ],
                    extensions: [
                        "Compare bones from athletes vs sedentary individuals (if available in museum/collection)",
                        "Investigate bones from different species (birds - hollow; mammals - solid) and relate to loading",
                        "Study osteoporotic bone (from medical education supplier) vs normal bone",
                        "Build computational model of femur and simulate remodeling over time",
                        "Research piezoelectric properties of bone (mechanical stress → electrical signals)"
                    ],
                    limitations: [
                        "Real bone observation is qualitative (trabecular pattern visible but not quantified)",
                        "Exercise studies in humans have confounding variables (nutrition, age, genetics)",
                        "Cannot measure strain in living human bones directly (usually done in animals or cadavers)",
                        "Remodeling takes months - difficult to observe in real-time in lab setting"
                    ]
                }
            },

            // ========================================
            // CALCIUM HOMEOSTASIS - FEEDBACK REGULATION
            // ========================================
            
            calcium_homeostasis_feedback: {
                name: "Calcium Homeostasis and Hormonal Feedback Regulation",
                relatedTopics: ['bone_remodeling', 'bone_structure'],
                category: 'physiology',
                historicalBackground: {
                    discovery: "Calcium's role in biological systems",
                    timeline: [
                        {year: "1883", scientist: "Sydney Ringer", discovery: "Calcium essential for heart muscle contraction"},
                        {year: "1909", scientist: "William MacCallum", discovery: "Parathyroid glands regulate blood calcium"},
                        {year: "1920s", scientists: "J.B. Collip", discovery: "Isolated parathyroid hormone (PTH)"},
                        {year: "1961", scientist: "D. Harold Copp", discovery: "Calcitonin (from thyroid C cells)"},
                        {year: "1970s", scientists: "Various", discovery: "Vitamin D is hormone; activated in kidney"}
                    ],
                    significance: "Understanding calcium regulation is critical for treating osteoporosis, kidney disease, parathyroid disorders, and vitamin D deficiency",
                    context: "Early 1900s: Doctors noticed tetany (muscle spasms) after accidental parathyroid removal during thyroid surgery. This led to discovery that parathyroids regulate calcium, which is essential for neuromuscular function.",
                    modern_understanding: "Calcium homeostasis involves three organs (bone, kidney, intestine) and three hormones (PTH, calcitonin, vitamin D) in complex feedback loops"
                },
                theory: {
                    importance_of_calcium: {
                        functions: [
                            "Muscle contraction (binds to troponin)",
                            "Neurotransmitter release (Ca²⁺ influx triggers exocytosis)",
                            "Blood clotting (cofactor for clotting factors)",
                            "Enzyme activity (cofactor and signaling molecule)",
                            "Bone structure (hydroxyapatite crystals)",
                            "Cell signaling (second messenger)"
                        ],
                        normal_range: "8.5-10.5 mg/dL (2.1-2.6 mM) in blood",
                        tight_regulation: "Even small deviations cause serious symptoms",
                        distribution: {
                            bone: "99% of body calcium (mainly as hydroxyapatite)",
                            extracellular_fluid: "0.9% (in blood and interstitial fluid)",
                            intracellular: "0.1% (kept very low by active pumping)"
                        }
                    },
                    hypocalcemia: {
                        definition: "Blood Ca²⁺ < 8.5 mg/dL",
                        causes: ["Hypoparathyroidism", "Vitamin D deficiency", "Kidney disease", "Magnesium deficiency"],
                        symptoms: [
                            "Muscle spasms, tetany, cramping",
                            "Tingling in fingers (paresthesias)",
                            "Chvostek's sign (facial twitching when tapped)",
                            "Trousseau's sign (hand spasm with BP cuff inflation)",
                            "Seizures (severe cases)",
                            "Cardiac arrhythmias"
                        ],
                        mechanism: "Low Ca²⁺ → hyperexcitability of neurons and muscle cells"
                    },
                    hypercalcemia: {
                        definition: "Blood Ca²⁺ > 10.5 mg/dL",
                        causes: ["Hyperparathyroidism", "Cancer (bone metastases or PTHrP secretion)", "Excess vitamin D", "Thiazide diuretics"],
                        symptoms: [
                            "'Stones, bones, groans, psychiatric overtones'",
                            "Kidney stones (calcium deposits)",
                            "Bone pain (excessive resorption)",
                            "Abdominal pain, constipation (groans)",
                            "Fatigue, confusion, depression (psychiatric)",
                            "Cardiac arrhythmias"
                        ],
                        mechanism: "High Ca²⁺ → decreased neuromuscular excitability; calcium deposits in soft tissues"
                    },
                    hormonal_regulation: {
                        PTH: {
                            source: "Chief cells of parathyroid glands (4 small glands behind thyroid)",
                            stimulus: "Low blood Ca²⁺ (detected by calcium-sensing receptors on parathyroid cells)",
                            targets_and_effects: {
                                bone: "Stimulates osteoclasts (indirectly via osteoblasts) → bone resorption → Ca²⁺ release",
                                kidney: "↑ Ca²⁺ reabsorption in distal tubule; ↓ phosphate reabsorption; activates vitamin D",
                                intestine: "Indirect (via vitamin D activation) → ↑ Ca²⁺ absorption"
                            },
                            result: "Blood Ca²⁺ increases",
                            feedback: "Negative feedback - rising Ca²⁺ inhibits PTH release",
                            half_life: "Minutes (rapid response)"
                        },
                        calcitonin: {
                            ...
                            targets_and_effects: {
                                ...
                                kidney: "↑ Ca²⁺ excretion in urine"
                            },
                            result: "Blood Ca²⁺ decreases",
                            importance: "Minor role in adults (significant in children); calcitonin deficiency (after thyroidectomy) does not cause hypercalcemia",
                            clinical: "Calcitonin used to treat Paget's disease and hypercalcemia"
                        },
                        vitamin_D: {
                            source: "Skin (UV converts 7-dehydrocholesterol to vitamin D₃) → liver (25-hydroxylase) → kidney (1α-hydroxylase, activated by PTH) → active form (calcitriol, 1,25-dihydroxyvitamin D₃)",
                            stimulus: "PTH (activates final step in kidney); low Ca²⁺ and low phosphate",
                            target: "Primarily intestine",
                            effect: "↑ Calcium and phosphate absorption in small intestine (upregulates calcium-binding protein calbindin)",
                            secondary_effects: "Also increases bone resorption and kidney reabsorption of Ca²⁺",
                            result: "Blood Ca²⁺ and phosphate increase",
                            deficiency: "Rickets (children - soft, deformed bones); osteomalacia (adults - bone pain, fractures)",
                            time_course: "Hours to days (slower than PTH)"
                        }
                    },
                    feedback_loops: {
                        hypocalcemia_response: [
                            "Blood Ca²⁺ drops → parathyroid glands detect via Ca²⁺-sensing receptors",
                            "PTH released → targets bone, kidney, and (via vitamin D) intestine",
                            "Bone: osteoclasts activated → Ca²⁺ released from bone",
                            "Kidney: ↑ Ca²⁺ reabsorption + vitamin D activation",
                            "Intestine: ↑ Ca²⁺ absorption (via vitamin D)",
                            "Blood Ca²⁺ rises → negative feedback inhibits PTH release"
                        ],
                        hypercalcemia_response: [
                            "Blood Ca²⁺ rises → detected by parathyroid and thyroid C cells",
                            "PTH release inhibited → ↓ bone resorption, ↓ kidney reabsorption, ↓ vitamin D activation",
                            "Calcitonin released (minor role) → inhibits osteoclasts",
                            "Blood Ca²⁺ decreases toward normal"
                        ]
                    }
                },
                labExperiment: {
                    title: "Simulating Calcium Homeostasis Feedback Loops",
                    hypothesis: "A negative feedback system can maintain calcium levels within a narrow range despite perturbations, mimicking physiological regulation by PTH and calcitonin",
                    purpose: "Model and understand the feedback mechanisms regulating blood calcium using a physical or computational simulation",
                    note: "Since we cannot ethically manipulate human calcium levels, this experiment uses models and data analysis",
                    materials: [
                        "Computer with spreadsheet software (Excel, Google Sheets) OR graphing calculator",
                        "Optional: Simulation software (e.g., STELLA, Vensim, or online physiology simulators)",
                        "Graph paper",
                        "Case study data (provided below or from literature)"
                    ],
                    procedure: [
                        "PART 1: Conceptual Model of Feedback Loop",
                        "",
                        "Draw a diagram of calcium homeostasis:",
                        "  Central box: Blood Calcium Level",
                        "  Input arrows: Bone resorption, Intestinal absorption, Kidney reabsorption",
                        "  Output arrows: Bone formation, Kidney excretion, Tissue deposition",
                        "  Control: PTH (increases inputs when Ca²⁺ low), Calcitonin (decreases inputs when Ca²⁺ high)",
                        "  Feedback: Blood Ca²⁺ level controls hormone release (negative feedback)",
                        "",
                        "Identify components:",
                        "  Sensor: Calcium-sensing receptors on parathyroid cells",
                        "  Control center: Parathyroid glands (release PTH), Thyroid C cells (release calcitonin)",
                        "  Effectors: Bone, kidney, intestine",
                        "  Set point: 8.5-10.5 mg/dL",
                        "",
                        "PART 2: Computational Simulation (Spreadsheet Model)",
                        "",
                        "Set up spreadsheet columns:",
                        "  Time (hours): 0, 1, 2, 3, ... 48",
                        "  Blood Ca²⁺ (mg/dL): starting at 10 mg/dL",
                        "  PTH level (arbitrary units): starting at baseline",
                        "  Bone resorption rate: depends on PTH",
                        "  Kidney reabsorption rate: depends on PTH",
                        "  Ca²⁺ loss rate: constant baseline excretion",
                        "  Net change in Ca²⁺: inputs - outputs",
                        "",
                        "Define relationships (simplified equations):",
                        "  If Ca²⁺ < 9 mg/dL: PTH increases proportionally (PTH = baseline × (10 - Ca²⁺))",
                        "  If Ca²⁺ > 10 mg/dL: PTH decreases, calcitonin increases",
                        "  Bone resorption = baseline + (PTH - baseline) × sensitivity_factor",
                        "  Kidney reabsorption = baseline + (PTH - baseline) × sensitivity_factor",
                        "  Net Ca²⁺ change = bone_resorption + kidney_reabsorption - Ca_loss - bone_formation",
                        "  New Ca²⁺ = previous Ca²⁺ + net_change",
                        "",
                        "Run simulations:",
                        "  Scenario 1: Sudden drop in Ca²⁺ to 7 mg/dL (e.g., ate no dairy for days)",
                        "    Observe: PTH rises → bone resorption and kidney reabsorption increase → Ca²⁺ returns to normal",
                        "  Scenario 2: Sudden rise in Ca²⁺ to 12 mg/dL (e.g., excess vitamin D)",
                        "    Observe: PTH suppressed, calcitonin rises → Ca²⁺ decreases toward normal",
                        "  Scenario 3: Parathyroid failure (PTH cannot increase)",
                        "    Observe: Ca²⁺ remains low (hypocalcemia)",
                        "  Scenario 4: Hyperparathyroidism (PTH constantly high)",
                        "    Observe: Ca²⁺ chronically elevated (hypercalcemia)",
                        "",
                        "Graph results:",
                        "  Plot Ca²⁺ vs time for each scenario",
                        "  Plot PTH vs time",
                        "  Observe oscillations, overshoot, time to equilibrium",
                        "",
                        "PART 3: Case Study Analysis (Real Patient Data)",
                        "",
                        "Analyze provided clinical scenarios:",
                        "",
                        "Case 1: Hypoparathyroidism",
                        "  Patient data: Ca²⁺ = 6.5 mg/dL (low), PTH = undetectable, phosphate = 6.0 mg/dL (high - normally PTH lowers phosphate)",
                        "  Symptoms: Muscle spasms, tingling",
                        "  Diagnosis: Parathyroid glands removed during thyroid surgery",
                        "  Treatment: Calcium and vitamin D supplements",
                        "  Question: Why doesn't calcitonin compensate? (Answer: Calcitonin only lowers Ca²⁺, can't raise it)",
                        "",
                        "Case 2: Primary Hyperparathyroidism",
                        "  Patient data: Ca²⁺ = 12.5 mg/dL (high), PTH = elevated, phosphate = 1.8 mg/dL (low)",
                        "  Symptoms: Kidney stones, bone pain, fatigue",
                        "  Diagnosis: Parathyroid adenoma (tumor secreting excess PTH)",
                        "  Treatment: Surgical removal of adenoma",
                        "  Question: Why is feedback not suppressing PTH? (Answer: Adenoma autonomous, ignores feedback)",
                        "",
                        "Case 3: Vitamin D Deficiency (Rickets)",
                        "  Patient data: Ca²⁺ = 7.8 mg/dL (low-normal), PTH = elevated, phosphate = low, vitamin D = very low",
                        "  Symptoms: Bowed legs, soft skull bones (child)",
                        "  Diagnosis: Inadequate sun exposure and dietary vitamin D",
                        "  Treatment: Vitamin D supplementation",
                        "  Mechanism: PTH is elevated (trying to compensate) but can't activate vitamin D (enzyme deficiency or lack of substrate) → poor intestinal Ca²⁺ absorption → inadequate mineralization",
                        "",
                        "For each case:",
                        "  Identify the broken component of feedback loop",
                        "  Predict compensatory responses (what's normal feedback system trying to do?)",
                        "  Explain why compensation is insufficient",
                        "  Propose treatment rationale"
                    ],
                    expectedResults: {
                        simulation: {
                            normal_feedback: "Ca²⁺ returns to set point (9-10 mg/dL) after perturbation; oscillations dampen over time",
                            hypocalcemia_response: "PTH spike → rapid Ca²⁺ increase from bone/kidney → stabilizes in hours",
                            hypercalcemia_response: "PTH suppression → slower Ca²⁺ decrease (calcitonin effect minimal in adults)",
                            broken_feedback: "Without PTH, Ca²⁺ stays low; with excess PTH, Ca²⁺ stays high"
                        },
                        case_studies: {
                            hypoparathyroidism: "Cannot raise Ca²⁺ without PTH; needs exogenous Ca²⁺ and vitamin D",
                            hyperparathyroidism: "Autonomous PTH secretion overrides feedback; only cure is remove source",
                            vitamin_D_deficiency: "PTH elevated but ineffective without vitamin D to act on intestine"
                        }
                    },
                    dataTable: [
                        ["Condition", "Blood Ca²⁺", "PTH", "Calcitonin", "Vitamin D", "Bone Effect"],
                        ["Normal", "9.5 mg/dL", "Normal", "Normal", "Normal", "Balanced remodeling"],
                        ["Hypocalcemia", "7.0 mg/dL", "High", "Low", "Activated", "Increased resorption"],
                        ["Hypercalcemia", "12.0 mg/dL", "Low", "High", "Suppressed", "Decreased resorption"],
                        ["Hypoparathyroidism", "6.5 mg/dL", "Low/Absent", "Low", "Not activated", "Decreased resorption"],
                        ["Hyperparathyroidism", "12.5 mg/dL", "High", "Suppressed", "Excess activated", "Excessive resorption"],
["Vitamin D Deficiency", "7.8 mg/dL", "High (compensatory)", "Low", "Very Low", "Increased resorption (inadequate)"]
],
graphing: [
"Plot blood Ca²⁺ vs time for different scenarios (normal feedback, no PTH, excess PTH)",
"Plot PTH level vs blood Ca²⁺ (negative correlation - shows feedback)",
"Plot bone resorption rate vs PTH level (positive correlation)",
"Create feedback loop diagram with arrows showing stimulation (+) and inhibition (-)"
],
quantitativeAnalysis: {
feedback_gain: "Measure how quickly system returns to set point after disturbance",
steady_state_error: "In normal system: minimal; in broken system: large deviation from set point",
time_constant: "Time to reach 63% of final value (typically hours for calcium)",
overshoot: "May occur if feedback is too strong or delayed"
},
molecularDetails: {
calcium_sensing_receptor: {
location: "Parathyroid chief cells (also kidney, bone)",
mechanism: "G-protein coupled receptor; activated by extracellular Ca²⁺",
response: "High Ca²⁺ → inhibits PTH secretion; Low Ca²⁺ → stimulates PTH secretion",
mutations: "Gain-of-function → hypocalcemia; loss-of-function → hypercalcemia"
},
PTH_signaling: {
receptor: "PTH1R (G-protein coupled receptor) on osteoblasts and kidney cells",
pathway: "PTH → cAMP/PKA and PLC/PKC pathways",
osteoblast_response: "Increase RANKL (stimulates osteoclast formation), decrease OPG",
kidney_response: "Activate 1α-hydroxylase (vitamin D), increase Ca²⁺ reabsorption channels"
},
vitamin_D_activation: {
step1: "Skin: 7-dehydrocholesterol + UVB → vitamin D₃ (cholecalciferol)",
step2: "Liver: vitamin D₃ + 25-hydroxylase → 25(OH)D₃ (calcidiol) [storage form]",
step3: "Kidney: 25(OH)D₃ + 1α-hydroxylase (↑ by PTH) → 1,25(OH)₂D₃ (calcitriol) [active form]",
target: "Calcitriol binds vitamin D receptor (VDR) in intestinal cells → transcription of calbindin (Ca²⁺ binding protein)"
}
},
analysis: {
feedback_characteristics: [
"Negative feedback: Rising Ca²⁺ inhibits PTH; falling Ca²⁺ stimulates PTH",
"Proportional response: Magnitude of PTH release proportional to deviation from set point",
"Multiple effectors: Bone, kidney, intestine provide redundancy",
"Hormonal cascade: PTH activates vitamin D, which acts on intestine (amplification)",
"Time scales: PTH acts in minutes-hours; vitamin D in hours-days; bone remodeling in weeks-months"
],
system_stability: [
"Robust: Multiple pathways to raise or lower Ca²⁺",
"Vulnerable: Failure of any one component (parathyroid, kidney, vitamin D) causes imbalance",
"Compensation: If one pathway fails, others try to compensate (e.g., high PTH in vitamin D deficiency)"
],
clinical_implications: [
"Diagnosis: Measure Ca²⁺, PTH, vitamin D, phosphate together to identify problem",
"Treatment: Replace missing component (PTH analogs, vitamin D, calcium supplements)",
"Prevention: Adequate vitamin D (sun exposure, diet), calcium intake, exercise (Wolff's Law)"
]
},
conclusions: [
"Calcium homeostasis is regulated by negative feedback involving PTH, calcitonin, and vitamin D",
"The system maintains blood Ca²⁺ within narrow range (8.5-10.5 mg/dL) despite dietary variations",
"Bone serves as calcium reservoir - resorbed when blood levels low, deposited when high",
"Feedback can be disrupted by parathyroid disease, vitamin D deficiency, or kidney failure",
"Understanding feedback loops is essential for diagnosing and treating calcium disorders"
],
realWorldApplications: [
"Osteoporosis treatment: Bisphosphonates (inhibit osteoclasts), PTH analogs (stimulate bone formation), vitamin D/calcium",
"Chronic kidney disease: Kidney can't activate vitamin D → secondary hyperparathyroidism → renal osteodystrophy",
"Parathyroid surgery: Remove adenoma to cure primary hyperparathyroidism",
"Rickets prevention: Vitamin D supplementation for infants, fortified milk",
"Hypercalcemia of malignancy: Cancers secrete PTHrP (PTH-related peptide) → high Ca²⁺"
],
extensions: [
"Investigate phosphate regulation (inverse relationship with Ca²⁺)",
"Research FGF23 (fibroblast growth factor 23) - regulates phosphate and vitamin D",
"Study calcitonin salmon (drug used for osteoporosis - more potent than human)",
"Model chronic kidney disease progression and its effect on Ca²⁺/phosphate balance",
"Investigate calcium's role in other systems (muscle, neurons, heart)"
]
}
},
// ========================================
        // BONE DENSITY MEASUREMENT - DEXA SCAN PRINCIPLES
        // ========================================
        
        bone_density_measurement: {
            name: "Bone Density Measurement: From X-rays to DEXA Technology",
            relatedTopics: ['bone_structure', 'bone_remodeling'],
            category: 'diagnostic_techniques',
            historicalBackground: {
                discovery: "X-rays and bone imaging",
                timeline: [
                    {year: "1895", scientist: "Wilhelm Röntgen", discovery: "Discovered X-rays; first image was his wife's hand showing bones"},
                    {year: "early 1900s", discovery: "Radiographs used to detect fractures and gross bone abnormalities"},
                    {year: "1963", scientist: "J. R. Cameron and J. Sorenson", discovery: "Single-photon absorptiometry (SPA) - first quantitative bone density measurement"},
                    {year: "1970s", discovery: "Dual-photon absorptiometry (DPA) developed for spine and hip"},
                    {year: "1987", discovery: "Dual-energy X-ray absorptiometry (DEXA/DXA) introduced - current gold standard"},
                    {year: "1994", discovery: "WHO defines osteoporosis based on DEXA T-score"}
                ],
                significance: "DEXA allows early detection of osteoporosis before fractures occur, enabling preventive treatment",
                context: "Before DEXA, osteoporosis often diagnosed only after fracture. Now we can measure bone density and predict fracture risk.",
                modern_use: "DEXA is standard of care for screening postmenopausal women, monitoring osteoporosis treatment, and assessing fracture risk"
            },
            theory: {
                x_ray_attenuation: {
                    principle: "X-rays absorbed by dense materials (bone) more than soft tissue",
                    beer_lambert_law: "I = I₀ × e^(-μx)",
                    where: {
                        I: "Transmitted intensity",
                        I₀: "Initial intensity",
                        μ: "Linear attenuation coefficient (depends on material density and composition)",
                        x: "Thickness of material"
                    },
                    bone_vs_soft_tissue: "Bone has higher attenuation coefficient (contains calcium) → appears white on X-ray; soft tissue is gray; air is black"
                },
                single_vs_dual_energy: {
                    single_energy: {
                        problem: "Can't distinguish bone from soft tissue thickness variations",
                        limitation: "If patient has more fat or muscle, signal changes even if bone density constant"
                    },
                    dual_energy: {
                        principle: "Use two different X-ray energies (high and low)",
                        advantage: "Bone and soft tissue attenuate the two energies differently → can mathematically separate them",
                        equations: "Two equations (one per energy), two unknowns (bone mass, soft tissue mass) → solve simultaneously",
                        result: "Accurate bone mineral density regardless of soft tissue thickness"
                    }
                },
                bone_mineral_density: {
                    definition: "BMD = bone mineral content (g) / area (cm²) = g/cm²",
                    note: "Actually areal density (2D projection), not true volumetric density",
                    limitation: "Larger bones appear more dense (size artifact) - important for children, large adults",
                    units: "g/cm² (e.g., 1.0 g/cm² for healthy young adult spine)"
                },
                T_score_and_Z_score: {
                    T_score: {
                        definition: "Number of standard deviations above/below mean BMD of healthy young adult (age 25-30)",
                        formula: "T-score = (patient BMD - young adult mean BMD) / young adult SD",
                        interpretation: {
                        ...
                        use: "Primary diagnostic criterion for osteoporosis (WHO definition)"
                    },
                    Z_score: {
                        definition: "Number of standard deviations above/below mean BMD of age-matched controls",
                        formula: "Z-score = (patient BMD - age-matched mean BMD) / age-matched SD",
                        use: "Detect unusually low bone density for age (suggests secondary cause)",
                        interpretation: "Z-score < -2.0 warrants investigation for underlying disease"
                    }
                },
                fracture_risk: {
                    relationship: "Each 1 SD decrease in BMD (T-score) approximately doubles fracture risk",
                    example: "T-score of -2.0 → ~4× fracture risk compared to T-score of 0",
                    sites: "Hip, spine, wrist most commonly fractured in osteoporosis",
                    FRAX: "Fracture Risk Assessment Tool - uses BMD plus clinical factors to predict 10-year fracture probability"
                }
            },
            labExperiment: {
                title: "Simulating X-ray Attenuation and Bone Density Measurement",
                hypothesis: "Materials with different densities attenuate radiation differently, allowing differentiation of bone from soft tissue, similar to DEXA scanning principles",
                purpose: "Demonstrate principles of X-ray attenuation and understand how DEXA distinguishes bone from soft tissue",
                note: "We will use visible light and filters instead of X-rays (safe alternative demonstrating same principles)",
                materials: [
                    "Bright light source (flashlight or lamp)",
                    "Light sensor (lux meter app on phone, or solar cell connected to multimeter)",
                    "Materials of varying density:",
                    "  - Clear plastic sheets (varying thicknesses)",
                    "  - Aluminum foil (1, 2, 3, 4 layers)",
                    "  - Cardboard (varying thicknesses)",
                    "  - Bone (chicken bone, dried and cleaned)",
                    "  - Fabric, rubber, foam (simulate soft tissue)",
                    "Ruler",
                    "Graph paper",
                    "Calculator",
                    "Optional: Real DEXA scan reports (anonymized) for analysis"
                ],
                safetyPrecautions: [
                    "Do not use actual X-ray equipment without proper training and licensing",
                    "Bright lights can be hot - allow to cool before touching",
                    "This experiment uses visible light as safe analog of X-rays"
                ],
                procedure: [
                    "PART 1: Demonstrating Attenuation (Beer-Lambert Law)",
                    "",
                    "Set up light source at fixed distance from light sensor",
                    "Measure baseline light intensity with no obstruction (I₀)",
                    "Place materials between source and sensor:",
                    "  Test 1: Varying thicknesses of same material (e.g., 1, 2, 3, 4 sheets plastic)",
                    "    Measure transmitted intensity (I) for each thickness (x)",
                    "    Record: thickness (mm), intensity (lux)",
                    "  Test 2: Varying materials of same thickness",
                    "    Measure intensity through: plastic, foil, cardboard, bone, fabric",
                    "    Record: material type, thickness, intensity",
                    "",
                    "Calculate attenuation:",
                    "  Transmission fraction: I/I₀",
                    "  Absorbance: -ln(I/I₀) = μx",
                    "  Attenuation coefficient: μ = -ln(I/I₀) / x",
                    "",
                    "Plot results:",
                    "  Plot ln(I/I₀) vs thickness (should be linear with slope -μ)",
                    "  Plot μ for different materials (bone should have highest μ - densest)",
                    "",
                    "PART 2: Simulating Dual-Energy Separation",
                    "",
                    "Use two different 'energies' (colors of light):",
                    "  Energy 1: White light (or blue LED)",
                    "  Energy 2: Red light (red LED or red filter)",
                    "",
                    "Create mixed sample:",
                    "  Layer 1: Thin cardboard (simulate soft tissue)",
                    "  Layer 2: Chicken bone section (simulate bone)",
                    "  Layer 3: More cardboard",
                    "",
                    "Measure attenuation at both 'energies':",
                    "  Record I₁/I₀ (white light) and I₂/I₀ (red light)",
                    "  Note: Different materials absorb red vs blue differently",
                    "",
                    "Demonstrate concept:",
                    "  Show that bone absorbs both colors strongly",
                    "  Soft tissue (cardboard) absorbs blue more than red",
                    "  Using both measurements, can calculate: 'bone' contribution and 'soft tissue' contribution",
                    "  (Actual math is complex; conceptual demonstration only)",
                    "",
                    "PART 3: Analyzing Real DEXA Data (if available)",
                    "",
                    "Obtain anonymized DEXA scan reports (from textbook, online resources, or clinical setting)",
                    "",
                    "Examine report components:",
                    "  BMD values (g/cm²) for: lumbar spine (L1-L4), femoral neck, total hip",
                    "  T-scores and Z-scores for each region",
                    "  Comparison to reference population (graph)",
                    "  Images showing bone density color map (white = dense, black = low density)",
                    "",
                    "Practice interpretation:",
                    "  Case 1: 75-year-old woman, L1-L4 BMD = 0.72 g/cm², T-score = -2.8",
                    "    Diagnosis: Osteoporosis (T-score < -2.5)",
                    "    Action: Consider treatment (bisphosphonates, calcium, vitamin D, exercise)",
                    "",
                    "  Case 2: 55-year-old woman, L1-L4 BMD = 0.95 g/cm², T-score = -1.3",
                    "    Diagnosis: Osteopenia (T-score between -1.0 and -2.5)",
                    "    Action: Lifestyle modifications, repeat scan in 1-2 years",
                    "",
                    "  Case 3: 30-year-old man, L1-L4 BMD = 1.15 g/cm², T-score = +0.5",
                    "    Diagnosis: Normal bone density",
                    "    Action: Maintain healthy lifestyle",
                    "",
                    "Calculate fracture risk:",
                    "  Use T-score to estimate relative risk",
                    "  Every 1 SD decrease (T-score decrease of 1) → ~2× risk",
                    "  Example: T-score -2.5 vs 0 → 2^2.5 = 5.7× higher fracture risk",
                    "",
                    "PART 4: Factors Affecting BMD",
                    "",
                    "Research and discuss:",
                    "  Age: Peak bone mass at ~30, declines thereafter",
                    "  Sex: Women have lower peak BMD and accelerated loss after menopause",
                    "  Ethnicity: African Americans have higher BMD than Caucasians and Asians",
                    "  Body size: Larger, heavier individuals have higher BMD (artifact and real)",
                    "  Exercise: Weight-bearing exercise increases BMD",
                    "  Nutrition: Calcium, vitamin D, protein intake",
                    "  Medications: Corticosteroids decrease BMD; bisphosphonates increase",
                    "  Medical conditions: Hyperparathyroidism, hyperthyroidism, celiac disease decrease BMD"
                ],
                expectedResults: {
                    attenuation: {
                        thickness_dependence: "Transmitted intensity decreases exponentially with thickness (I = I₀e^(-μx))",
                        material_dependence: "Denser materials (bone) have higher attenuation coefficient μ than less dense (soft tissue)",
                        bone: "Highest attenuation - blocks most light",
                        soft_tissue: "Intermediate attenuation",
                        air: "Minimal attenuation"
                    },
                    dual_energy: {
                        concept: "Two different 'energies' attenuated differently by bone vs soft tissue allows separation",
                        bone: "High attenuation at both energies, but ratio differs from soft tissue",
                        soft_tissue: "Lower attenuation, different energy-dependent ratio"
                    },
                    DEXA_interpretation: {
                        osteoporosis: "T-score ≤ -2.5; high fracture risk",
                        osteopenia: "T-score -1.0 to -2.5; moderate risk",
                        normal: "T-score ≥ -1.0; low risk"
                    }
                },
                dataTable: [
                    ["Material", "Thickness (mm)", "Transmitted Intensity (lux)", "I/I₀", "Attenuation Coeff μ"],
                    ["None (baseline)", "0", "1000", "1.00", "-"],
                    ["Plastic", "1", "850", "0.85", "0.16"],
                    ["Plastic", "2", "720", "0.72", "0.16"],
                    ["Plastic", "3", "610", "0.61", "0.16"],
                    ["Aluminum foil", "0.02", "750", "0.75", "14.4"],
                    ["Cardboard", "2", "600", "0.60", "0.26"],
                    ["Bone (chicken)", "3", "250", "0.25", "0.46"],
                    ["", "", "", "", ""],
                    ["DEXA Scan Examples", "", "", "", ""],
                    ["Patient", "Age/Sex", "BMD (g/cm²)", "T-score", "Diagnosis"],
                    ["A", "75F", "0.72", "-2.8", "Osteoporosis"],
                    ["B", "55F", "0.95", "-1.3", "Osteopenia"],
                    ["C", "30M", "1.15", "+0.5", "Normal"],
                    ["D", "80M", "0.88", "-1.8", "Osteopenia"]
                ],
                calculations: {
                    attenuation_coefficient: "μ = -ln(I/I₀) / thickness",
                    T_score: "(Patient BMD - Young adult mean) / Young adult SD",
                    fracture_risk: "Relative risk ≈ 2^(-T-score)",
                    example: {
                        patient_BMD: "0.85 g/cm²",
                        young_adult_mean: "1.00 g/cm²",
                        SD: "0.10 g/cm²",
                        T_score: "(0.85 - 1.00) / 0.10 = -1.5",
                        diagnosis: "Osteopenia",
                        relative_risk: "2^1.5 = 2.8× fracture risk vs T-score 0"
                    }
                },
                clinicalGuidelines: {
                    screening_recommendations: {
                        women: "All women ≥65 years; younger postmenopausal women with risk factors",
                        men: "Men ≥70 years; younger men with risk factors",
                        risk_factors: ["Previous fracture", "Family history", "Smoking", "Low body weight", "Corticosteroid use"]
                    },
                    monitoring: {
                        untreated: "Repeat DEXA every 1-2 years for osteopenia, 2-3 years if normal",
                        treated: "Repeat after 1-2 years of osteoporosis treatment to assess response"
                    },
                    treatment_thresholds: {
                        pharmacologic: "T-score ≤ -2.5, or T-score -1.0 to -2.5 with high fracture risk (FRAX)",
                        non_pharmacologic: "All patients: calcium 1000-1200 mg/day, vitamin D 800-1000 IU/day, weight-bearing exercise, fall prevention"
                    }
                },
                analysis: {
                    DEXA_advantages: [
                        "Low radiation dose (~1/10 of chest X-ray)",
                        "Fast scan (~10 minutes)",
                        "High precision (1-2% error)",
                        "Predicts fracture risk",
                        "Widely available"
                    ],
                    DEXA_limitations: [
                        "Areal density (g/cm²), not true volumetric density",
                        "Size artifact: larger bones appear denser",
                        "Cannot assess bone quality (only quantity)",
                        "Degenerative changes (arthritis) can falsely elevate spine BMD",
                        "Doesn't detect recent bone loss (months lag)"
                    ],
                    alternative_methods: {
                        QCT: "Quantitative CT - true volumetric density, but higher radiation",
                        ultrasound: "Heel ultrasound - portable, no radiation, but less accurate",
                        MRI: "Research tool; can assess bone microarchitecture",
                        biomarkers: "Bone turnover markers (blood/urine) - assess remodeling rate"
                    }
                },
                conclusions: [
                    "DEXA uses differential X-ray attenuation to measure bone mineral density",
                    "Dual-energy approach separates bone from soft tissue for accurate measurement",
                    "T-score < -2.5 defines osteoporosis and indicates high fracture risk",
                    "DEXA enables early detection and treatment of osteoporosis before fractures occur",
                    "Bone density is modifiable - exercise, nutrition, and medications can improve BMD"
                ],
                realWorldApplications: [
                    "Osteoporosis screening in postmenopausal women and elderly men",
                    "Monitoring response to osteoporosis treatment (bisphosphonates, denosumab, teriparatide)",
                    "Assessing fracture risk in patients on chronic corticosteroids",
                    "Detecting bone loss in athletes with eating disorders (female athlete triad)",
                    "Research: studying effects of nutrition, exercise, medications on bone health"
                ],
                extensions: [
                    "Investigate trabecular bone score (TBS) - assesses bone microarchitecture from DEXA images",
                    "Research FRAX calculator - integrates BMD with clinical factors for personalized fracture risk",
                    "Study vertebral fracture assessment (VFA) - lateral spine imaging to detect vertebral fractures",
                    "Analyze cost-effectiveness of DEXA screening programs",
                    "Investigate new imaging methods: HR-pQCT (high-resolution peripheral QCT) for bone microstructure"
                ]
            }
        },

        // ========================================
        // FORENSIC ANTHROPOLOGY - SEX AND AGE DETERMINATION
        // ========================================
        
        forensic_skeletal_analysis: {
            name: "Forensic Anthropology: Sex and Age Determination from Skeletal Remains",
            relatedTopics: ['axial_skeleton', 'appendicular_skeleton', 'bone_development'],
            category: 'applied_anatomy',
            historicalBackground: {
                development: "Forensic anthropology as a science",
                timeline: [
                    {year: "1800s", development: "Early anatomists noted sex differences in pelvis and skull"},
                    {year: "1920s-1930s", scientist: "T. Wingate Todd", contribution: "Developed age estimation methods from pubic symphysis"},
                    {year: "1950s", scientist: "Wilton Marion Krogman", contribution: "Published 'The Human Skeleton in Forensic Medicine' - foundational text"},
                    {year: "1960s-1970s", development: "Systematic studies of skeletal populations; standardized methods"},
                    {year: "1980s-present", development: "Application to mass disasters, human rights investigations, criminal cases"}
                ],
                significance: "Skeletal analysis can establish biological profile (sex, age, ancestry, stature) from remains, aiding identification",
                context: "When only bones remain (archaeological sites, crime scenes, mass disasters), skeletal features reveal individual's characteristics",
                modern_applications: "Identify victims of natural disasters, war crimes, terrorist attacks; solve criminal cases; study ancient populations"
            },
            theory: {
                biological_profile: {
                    components: ["Sex", "Age", "Ancestry", "Stature", "Individualizing features (trauma, pathology, anomalies)"],
                    goal: "Narrow possible identities; compare to missing persons reports"
                },
                sex_determination: {
                    dimorphism: "Males generally larger, more robust; females smaller, more gracile",
                    best_indicators: "Pelvis (95% accuracy), skull (90% accuracy)",
                    reliability: "High if adult skeleton complete; poor in children (puberty creates sex differences)",
                    pelvis_most_reliable: "Pelvis adapted for childbirth in females → dramatic differences"
                },
                age_estimation: {
                    children: "Very accurate using dental development and bone ossification",
                    adults: "Less accurate; based on degenerative changes (error ±5-10 years)",
                    methods: "Pubic symphysis, auricular surface, cranial sutures, dental wear, rib ends"
                },
                ancestry: {
                    note: "Skeletal features correlate with geographic ancestry, not race (social construct)",
                    features: "Skull shape, nasal aperture, facial structure",
                    major_groups: "African, European, Asian, Native American (broad categories with overlap)",
                    caution: "Population variation and admixture limit accuracy; should not overstate certainty"
                },
                stature: {
                    method: "Regression equations using long bone lengths (femur, tibia, humerus)",
                    accuracy: "±3-4 cm in adults",
                    population_specific: "Equations differ by sex and ancestry"
                }
            },
            labExperiment: {
                title: "Skeletal Analysis: Determining Sex and Age from Bone Features",
                hypothesis: "Specific skeletal features (pelvic morphology, skull characteristics, bone fusion) can be used to determine biological sex and estimate age at death",
                purpose: "Learn forensic anthropology methods for establishing biological profile from skeletal remains",
                materials: [
                    "Skeletal models or casts (male and female pelvis, male and female skulls)",
                    "Alternatively: high-quality photographs or diagrams",
                    "Real bones if available (from anatomical teaching collection)",
                    "Calipers or ruler",
                    "Magnifying glass",
                    "Reference charts for age estimation (provided below)",
                    "Data sheets",
                    "Optional: 3D printed bones, virtual skeleton software"
                ],
                safetyPrecautions: [
                    "Handle bones respectfully (if human remains)",
                    "Wash hands after handling specimens",
                    "Follow institutional ethics guidelines for human remains"
                ],
                procedure: [
                    "PART 1: Sex Determination from Pelvis",
                    "",
                    "Examine pelvis (os coxae) - MOST RELIABLE sex indicator",
                    "",
                    "Feature 1: Subpubic angle (angle between inferior pubic rami)",
                    "  Male: Narrow, <70° (acute angle, 'V-shaped')",
                    "  Female: Wide, >80° (obtuse angle, 'U-shaped')",
                    "  Measure with protractor or estimate visually",
                    "",
                    "Feature 2: Greater sciatic notch (posterior pelvis)",
                    "  Male: Narrow, <68° (looks like narrow 'V')",
                    "  Female: Wide, >68° (looks like wide 'U')",
                    "  Measure angle or use reference diagrams",
                    "",
                    "Feature 3: Pelvic inlet shape (superior view)",
                    "  Male: Heart-shaped, narrow",
                    "  Female: Oval or circular, wide (childbirth adaptation)",
                    "",
                    "Feature 4: Preauricular sulcus (near sacroiliac joint)",
                    "  Male: Absent or very slight",
                    "  Female: Often deep groove (pregnancy/childbirth related)",
                    "",
                    "Feature 5: Obturator foramen shape",
                    "  Male: Round",
                    "  Female: Oval (wider side-to-side)",
                    "",
                    "Feature 6: Acetabulum (hip socket) size",
                    "  Male: Larger diameter (larger femoral head)",
                    "  Female: Smaller",
                    "",
                    "Feature 7: Overall size and robusticity",
                    "  Male: Larger, thicker bones, rougher muscle attachments",
                    "  Female: Smaller, more gracile, smoother",
                    "",
                    "Score each feature (1=male, 2=ambiguous, 3=female)",
                    "Average score: <2 = male, >2 = female, ~2 = indeterminate",
                    "",
                    "PART 2: Sex Determination from Skull",
                    "",
                    "Feature 1: Mastoid process (behind ear)",
                    "  Male: Large, projecting",
                    "  Female: Small, less projecting",
                    "",
                    "Feature 2: Supraorbital ridge (brow ridge)",
                    "  Male: Prominent, heavy ridge",
                    "  Female: Minimal or absent ridge, smoother forehead",
                    "",
                    "Feature 3: Nuchal crest (back of skull, occipital bone)",
                    "  Male: Prominent ridge, rough (large neck muscles)",
                    "  Female: Smoother, less pronounced",
                    "",
                    "Feature 4: Mandible (jaw)",
                    "  Male: Square chin, large, robust, gonial angle ~90°",
                    "  Female: Pointed chin, smaller, gracile, gonial angle >90°",
                    "",
                    "Feature 5: Forehead",
                    "  Male: Sloping",
                    "  Female: More vertical, rounder",
                    "",
                    "Feature 6: Overall size",
                    "  Male: Larger, heavier",
                    "  Female: Smaller, lighter",
                    "",
                    "Feature 7: Orbits (eye sockets)",
                    "  Male: Smaller, square",
                    "  Female: Larger, rounder",
                    "",
                    "Score and average as with pelvis",
                    "",
                    "PART 3: Age Estimation in Adults",
                    "",
                    "Method 1: Pubic Symphysis (Suchey-Brooks method)",
                    "  Examine surface of pubic symphysis (where two pubic bones meet anteriorly)",
                    "  Age progression:",
                    "    Phase I (18-23 yrs): Billowing surface, no rim",
                    "    Phase II (20-30): Reduction of billowing, rim begins",
                    "    Phase III (25-40): Rim complete, face relatively smooth",
                    "    Phase IV (30-50): Rim irregular, face eroded",
                    "    Phase V (35-60): Rim deteriorating, face porous",
                    "    Phase VI (45-85+): Rim eroded away, face extremely porous",
                    "  Compare specimen to reference casts/images",
                    "  Record estimated age range",
                    "",
                    "Method 2: Auricular Surface (Lovejoy method)",
                    "  Examine auricular surface (ear-shaped area on ilium that articulates with sacrum)",
                    "  Young (20-30): Fine granular texture, no porosity",
                    "  Middle (30-50): Surface becomes denser, porosity increases",
                    "  Old (50+): Dense, irregular, porous with lipping",
                    "",
                    "Method 3: Cranial Suture Closure",
                    "  Examine degree of fusion of skull sutures",
                    "  Young adult (20-30): Sutures open, clearly visible",
                    "  Middle age (30-50): Sutures begin to fuse (obliterate)",
                    "  Older (50+): Sutures fully fused (synostosis)",
                    "  CAUTION: High variability, use with other methods only",
                    "",
                    "Method 4: Dental Wear",
                    "  Examine tooth wear patterns",
                    "  Young: Minimal wear, cusps sharp",
                    "  Middle: Moderate wear, dentin exposed",
                    "  Old: Severe wear, teeth worn to roots",
                    "  NOTE: Diet-dependent; less reliable in modern populations",
                    "",
                    "Combine methods for best age estimate (e.g., 'Adult, 35-50 years')",
                    "",
                    "PART 4: Age Estimation in Children (Subadults)",
                    "",
                    "Method 1: Dental Development (MOST ACCURATE)",
                    "  Chart which teeth have erupted",
                    "  Reference:",
                    "    0-6 months: No teeth",
                    "    6-12 months: Central incisors erupt",
                    "    12-18 months: First molars",
                    "    18-24 months: Canines",
                    "    2-3 years: Second molars (complete primary dentition)",
                    "    6 years: First permanent molars",
                    "    6-7 years: Central incisors",
                    "    10-12 years: Premolars, canines",
                    "    12-13 years: Second molars",
                    "    17-21 years: Third molars (wisdom teeth)",
                    "  Accuracy: ±1-2 years",
                    "",
                    "Method 2: Epiphyseal Fusion",
                    "  Examine whether growth plates (epiphyseal plates) are open or fused",
                    "  Examples:",
                    "    Proximal humerus: Fuses ~20 years",
                    "    Distal femur: Fuses ~18 years",
                    "    Iliac crest: Fuses ~23 years",
                    "    Medial clavicle: Fuses ~25 years (LAST to fuse)",
                    "  If epiphysis not fused → individual died before that age",
                    "",
                    "Method 3: Bone Length",
                    "  Measure long bone lengths, compare to growth charts",
                    "  Estimate age from diaphyseal length (accurate in fetal-child, less so in adolescents)",
                    "",
                    "PART 5: Practice Cases",
                    "",
                    "Case 1: Unknown remains found, complete skeleton",
                    "  Pelvis: Wide subpubic angle (85°), wide greater sciatic notch, preauricular sulcus present",
                    "  Skull: Small mastoid, no brow ridge, pointed chin",
                    "  Pubic symphysis: Phase III (smooth face, complete rim)",
                    "  Conclusion: Female, age 25-40 years",
                    "",
                    "Case 2: Partial skeleton, skull and long bones only",
                    "  Skull: Large mastoid, prominent brow ridge, sloping forehead",
                    "  Sutures: Partially fused",
                    "  Femur length: 48 cm → stature estimate ~175 cm (5'9\")",
                    "  Conclusion: Male, age 35-55 years, tall stature",
                    "",
                    "Case 3: Child skeleton",
                    "  Dentition: First permanent molars present, second primary molars still present",
                    "  Epiphyses: All unfused",
                    "  Conclusion: Child, age 6-8 years",
                    "",
                    "For each case: Record sex determination features, age indicators, final biological profile"
                ],
                expectedResults: {
                    sex_determination: {
                        pelvis: "95%+ accuracy if complete adult pelvis",
                        skull: "~90% accuracy if complete adult skull",
                        combined: ">98% accuracy if both pelvis and skull available",
                        children: "Cannot reliably determine sex (minimal dimorphism before puberty)"
                    },
                    age_estimation: {
                        children: "Very accurate (±1-2 years) using dental development",
                        young_adults: "Moderate accuracy (±5-7 years) using pubic symphysis",
                        older_adults: "Lower accuracy (±10-15 years) due to individual variation",
                        elderly: "Broad ranges only (e.g., '60+ years')"
                    }
                },
                dataTable: [
                    ["Feature", "Male", "Female", "Score (1=M, 3=F)"],
                    ["PELVIS", "", "", ""],
                    ["Subpubic angle", "<70° (acute)", ">80° (obtuse)", ""],
                    ["Greater sciatic notch", "<68° (narrow)", ">68° (wide)", ""],
                    ["Pelvic inlet", "Heart-shaped", "Oval/circular", ""],
                    ["Preauricular sulcus", "Absent", "Present", ""],
                    ["Obturator foramen", "Round", "Oval", ""],
                    ["", "", "", ""],
                    ["SKULL", "", "", ""],
                    ["Mastoid process", "Large, projecting", "Small", ""],
                    ["Supraorbital ridge", "Prominent", "Minimal/absent", ""],
                    ["Nuchal crest", "Prominent", "Smooth", ""],
                    ["Mandible", "Square, robust", "Pointed, gracile", ""],
                    ["Forehead", "Sloping", "Vertical", ""],
                    ["", "", "", ""],
                    ["AGE INDICATORS (Adults)", "Young", "Old", "Estimate"],
                    ["Pubic symphysis", "Billowing, no rim", "Eroded, porous", ""],
                    ["Auricular surface", "Fine, granular", "Dense, porous", ""],
                    ["Cranial sutures", "Open", "Fused", ""],
                    ["Dental wear", "Minimal", "Severe", ""]
                ],
                sexDeterminationScoring: {
                    pelvis_features: {
                        subpubic_angle: {score1: "<70° (male)", score2: "70-80° (ambiguous)", score3: ">80° (female)"},
                        sciatic_notch: {score1: "<68° (male)", score2: "68-74° (ambiguous)", score3: ">74° (female)"},
                        preauricular_sulcus: {score1: "Absent (male)", score2: "Slight", score3: "Deep (female)"},
                        overall_shape: {score1: "Narrow, robust", score2: "Intermediate", score3: "Wide, gracile"}
                    },
                    interpretation: {
                        average_score_below_1_5: "Probable male",
                        average_score_1_5_to_2_5: "Indeterminate",
                        average_score_above_2_5: "Probable female",
                        average_score_below_1_2_or_above_2_8: "Definite sex assignment (high confidence)"
                    }
                },
                ageEstimationMethods: {
                    pubic_symphysis: {
                        phase_I: "18-23 years - billowing surface",
                        phase_II: "20-30 years - rim begins",
                        phase_III: "25-40 years - rim complete",
                        phase_IV: "30-50 years - rim irregular",
                        phase_V: "35-60 years - rim deteriorating",
                        phase_VI: "45-85+ years - rim eroded"
                    },
                    dental_eruption: {
                        first_molars_6yrs: "Age ~6 years",
                        central_incisors_6_7yrs: "Age 6-7 years",
                        premolars_10_12yrs: "Age 10-12 years",
                        second_molars_12_13yrs: "Age 12-13 years",
                        third_molars_17_21yrs: "Age 17-21 years (highly variable)"
                    },
                    epiphyseal_fusion: {
                        distal_radius: "Fuses ~18-19 years",
                        proximal_humerus: "Fuses ~20 years",
                        distal_femur: "Fuses ~18 years",
                        iliac_crest: "Fuses ~23 years",
                        medial_clavicle: "Fuses ~25 years (LAST)"
                    }
                },
                analysis: {
                    strengths: [
                        "Non-invasive (doesn't destroy evidence)",
                        "Can analyze very old remains (bones preserve well)",
                        "Pelvis provides very high accuracy for sex",
                        "Dental age estimation in children extremely accurate",
                        "Multiple independent methods increase reliability"
                    ],
                    limitations: [
                        "Requires expertise and training",
                        "Incomplete skeletons reduce accuracy",
                        "Children cannot be sexed reliably from skeleton",
                        "Age estimates in adults have wide error margins",
                        "Population variation (methods developed on specific populations may not apply universally)",
                        "Pathology and trauma can obscure features"
                    ],
                    sources_of_error: [
                        "Observer error (subjective assessment)",
                        "Population differences (methods may not generalize)",
                        "Individual variation (some people don't fit typical patterns)",
                        "Preservation (weathering, burning can alter features)",
                        "Incomplete remains (missing key bones)"
                    ]
                },
                conclusions: [
                    "Pelvis is most reliable indicator of biological sex (95%+ accuracy)",
                    "Age can be estimated accurately in children using dental development",
                    "Age estimation in adults is less accurate but provides useful range",
                    "Multiple skeletal features must be assessed together for best results",
                    "Forensic anthropology provides crucial information for identifying unknown remains"
                ],
                realWorldApplications: [
                    "Identifying disaster victims (plane crashes, earthquakes, tsunamis)",
                    "Criminal investigations (homicides, missing persons)",
                    "Human rights investigations (mass graves, war crimes)",
                    "Historical archaeology (studying ancient populations)",
                    "Repatriation of remains (Native American Graves Protection Act)"
                ],
                extensions: [
                    "Investigate stature estimation from long bone lengths (regression equations)",
                    "Study ancestry assessment (cranial measurements, facial features)",
                    "Research traumatic injuries visible on bones (gunshot wounds, sharp force, blunt force)",
                    "Learn about taphonomy (postmortem changes to bones)",
                    "Explore isotope analysis for diet and geographic origin",
                    "Investigate facial reconstruction from skulls"
                ],
                ethicalConsiderations: [
                    "Respect for human remains",
                    "Collaboration with descendant communities",
                    "Proper curation and storage",
                    "Informed consent for teaching specimens",
                    "Repatriation issues (NAGPRA in US)"
                ]
            }
        }
    };
...
linkExperimentsToTopics() {
    Object.entries(this.skeletalExperiments).forEach(([expId, experiment]) => {
        experiment.relatedTopics.forEach(topicId => {
            if (this.skeletalTopics[topicId]) {
                if (!this.skeletalTopics[topicId].relatedExperiments) {
                    this.skeletalTopics[topicId].relatedExperiments = [];
                }
                this.skeletalTopics[topicId].relatedExperiments.push({
                    id: expId,
