...
export async function GET(request) {
  try {
    ...
    const typesToFetch = viewType === 'all'
      ? Object.keys(TYPE_TO_ARRAY)
      : [viewType]
    ...
    typesToFetch.forEach(type => {
      ...
      rawItems.forEach((item, index) => {
        // Only include items with a rendered file that exists
        ...
        allItems.push({
          ...
          size: fileExists ? `${Math.round(fs.statSync(item.filename).size / 1024)}KB` : 'N/A'
        })
      })
    })
