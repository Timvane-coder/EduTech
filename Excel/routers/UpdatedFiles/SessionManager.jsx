...
export default function SessionManager({
  ...
  const addBiochemicalDiagram = async (config) => {
    setIsProcessing(true)
    try {
      ...
      if (!response.ok) throw new Error('Failed to add biochemical diagram')
      const { diagram, state } = await response.json()
      setWorkbookState(state)
      onStateChange?.(state)
      return diagram
    } catch (error) {
      console.error('Failed to add biochemical diagram:', error)
      throw error
    } finally {
      setIsProcessing(false)
    }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      window.sessionManager = {
        ...
        addGeometricShape,
        addGraph,
        addBiochemicalDiagram,
        exportToPNG,
        exportToExcel,
        resetWorkbook,
        cleanupSession: async () => { console.log('Dev mode: skipping session cleanup') },
    ...
    return () => { if (typeof window !== 'undefined') delete window.sessionManager }
