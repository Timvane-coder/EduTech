import { BiochemicalDiagramsRegistry } from '@/lib/worksheet'
...
class ServerWorkbookManager {
  ...
  async loadData(sessionId, data) {
    ...
    if (!workbook) throw new Error(`Workbook not found for session: ${sessionId}`)
  ...
  async applyFormula(sessionId, targetCell, formulaKey, params) {
    ...
    return { result, state: this.getCurrentState(sessionId) }
  }
  ...
  getCurrentState(sessionId) {
    ...
    // Defensive checks — guards against stale workbook instances after hot reload
    const safeLength = (arr) => (Array.isArray(arr) ? arr.length : 0)

    let graphCounts = {
      equations: 0, triangles: 0, circles: 0, rectangles: 0, squares: 0,
      parallelograms: 0, polygons: 0, ellipses: 0, quadrilaterals: 0,
      trapezoids: 0, spheres: 0, cylinders: 0, cones: 0, cubes: 0,
      vectors: 0, matrices: 0, total: 0
    }
    ...
    if (workbook.graphingCalculator) {
      ...
      graphCounts.total = Object.values(graphCounts).reduce((s, c) => s + c, 0)
    }
    ...
    return {
      ...
      charts:             safeLength(workbook.charts),
      biochemicalDiagrams: safeLength(workbook.biochemicalDiagrams),
  ...
  async exportToPNG(sessionId, options = {}) {
    const workbook = this.getWorkbook(sessionId)
    if (!workbook) throw new Error('Workbook not found')
    return workbook.exportToPNG(null, options)
  }
...
// Hot-reload safe singleton — re-evaluating this module won't wipe existing workbooks
