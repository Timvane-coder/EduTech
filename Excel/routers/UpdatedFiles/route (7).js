//app/api/workbook/visualizations/delete/route.js
...
const TYPE_TO_REMOVE = {
  ...
  biochemical:     (wb, i) => wb.removeBiochemicalDiagram(i)
}
...
export async function DELETE(request) {
  try {
    ...
    if (item.filename && fs.existsSync(item.filename)) {
      try { fs.unlinkSync(item.filename) } catch (e) { console.warn('File delete failed:', e.message) }
    }
