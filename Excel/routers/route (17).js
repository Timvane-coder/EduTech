//api/workbook/render-visualizations/route.js
...
export async function GET(request) {
  try {
    ...
    if (!sessionId) {
      // Return empty array if no session — ViewTab calls this without sessionId sometimes
    ...
    if (shouldInclude('charts')) {
      workbook.charts.forEach((item, index) => {
        visualizations.push({
          id: item.id,
          title: item.title,
          type: 'charts',
