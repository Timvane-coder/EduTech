...
// ── Remove all existing flat walls ───────────────────────────────────────────
const wallNames = ['wall_front','wall_back','wall_left','wall_right','roof_top','roof_underside','roof_fascia','ceiling','roof']
wallNames.forEach(name => {
  const n = root.listNodes().find(n => n.getName() === name)
  if (n) { n.setMesh(null); n.dispose(); console.log(`✅ Removed "${name}"`) }
})
