...
// ── Empty answer lines ─────────────────────────────────────────────────────
function answerLine(widthDxa = DXA.CONTENT_WIDTH - DXA.MARKS_COL) {
...
function answerLines(count = 4, widthDxa) {
    return Array.from({ length: count }, () => answerLine(widthDxa));
}
...
renderPage(subject, pageKey, options = {}, drawingStep = 9999) {
    ...
    switch (pageKey) {
        ...
        case 'questionSectionCPage':
            this.renderQuestionSectionPage(subject, pageKey, page, opts, resolvedStep);
            break;
        case 'closingPage':
    ...
    renderQuestionSectionPage(subject, pageKey, page, opts, drawingStep) {
        ...
        if (step >= 4) {
            ...
            questions.forEach((q, qi) => {
                this._renderQuestionBlock(q, qi, opts, step);
            });
        }
    ...
    _renderQuestionBlock(q, qi, opts, step) {
        ...
        // Context text for this specific question
        if (q.context) {
            stemContent.push(
                new Paragraph({
                    spacing:  spacing(40, 60),
        ...
        const marksText = step >= 5
            ? `[${q.marks || q.totalMarks || '—'}]`
            : '';

        if (marksText) {
            this._append(marksTable(stemContent, marksText));
        } else {
            this._append(...stemContent);
        }

        // ── Sub-parts ──────────────────────────────────────────────────────
        if (q.subParts && q.subParts.length) {
            q.subParts.forEach((sp, si) => {
                const letter      = String.fromCharCode(97 + si);   // a, b, c …
                const spMarksText = step >= 5 ? `[${sp.marks || '—'}]` : '';
                ...
                if (spMarksText) {
                    this._append(marksTable(spContent, spMarksText));
                } else {
                    this._append(...spContent);
                }

                // Answer lines for sub-part
                if (step >= 7 && opts.showAnswerLines !== false) {
                    const linesCount = sp.answerLines || opts.answerLinesPerQuestion || 4;
                    this._append(...answerLines(linesCount));
                }
            });
        } else {
            // No sub-parts — answer lines directly under question
            if (step >= 7 && opts.showAnswerLines !== false) {
                this._append(...answerLines(q.answerLines || opts.answerLinesPerQuestion || 4));
            }
        }

        // ── Diagram space for this question ───────────────────────────────
...
renderFullPaper(subject, overrideOptions = {}) {
    ExaminationPaperRegistry._assertSubject(subject);
    ...
    // ① Cover
    this.renderPage(subject, 'coverPage', overrideOptions);

    // ② Section A
    const sectionAQuestions = _getQuestions('questionSectionAPage');
    this.renderPage(subject, 'questionSectionAPage', {
        ...overrideOptions,
    ...
    this.renderPage(subject, 'questionSectionBPage', {
        ...overrideOptions,
    ...
    if (sectionCQuestions.length) {
        this.renderPage(subject, 'questionSectionCPage', {
            ...overrideOptions,
    ...
    // ⑤ Closing
    this.renderPage(subject, 'closingPage', overrideOptions);
