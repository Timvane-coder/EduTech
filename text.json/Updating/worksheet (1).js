...
export class EnhancedSpreadsheetWorkbook {
    constructor(options = {}) {
        ...
        this.graphRenderer = new GraphRenderer(null);
         ...
         // Biochemical diagram management
        this.biochemicalDiagrams = [];
        this.biochemicalRenderer = new BiochemicalDiagramRenderer(null);
...
// ========================================================================
    // BIOCHEMICAL DIAGRAM MANAGEMENT METHODS
    // ========================================================================

    addBiochemicalDiagram(diagramConfig) {
        const {
            key,
            title    = null,
            options  = {},
        ...
        const diagram = BiochemicalDiagramsRegistry.getDiagram(key);
        if (!diagram) throw new Error(`Biochemical diagram '${key}' not found`);

        const diagramObj = {
            id:        `biochemical_${this.biochemicalDiagrams.length + 1}`,
            key,
            title:     title || diagram.name,
            category:  diagram.category,
            options:   { ...options },
            filename:  filename || `${this.sheetName}_biochemical_${key}_${Date.now()}.png`,
            createdAt: new Date()
        };

        this.biochemicalDiagrams.push(diagramObj);
        this.addToHistory(`Added biochemical diagram: ${diagramObj.title}`);
        return diagramObj;
    }

    renderBiochemicalDiagramToPNG(diagramIndex) {
        if (diagramIndex < 0 || diagramIndex >= this.biochemicalDiagrams.length) {
            throw new Error(`Biochemical diagram index ${diagramIndex} out of range`);
        }

        const diagramConfig = this.biochemicalDiagrams[diagramIndex];
        const width  = diagramConfig.options.width  || 720;
        const height = diagramConfig.options.height || 560;

        const canvas = createCanvas(width, height);
        const ctx    = canvas.getContext('2d');

        this.biochemicalRenderer.canvas = canvas;
        this.biochemicalRenderer.ctx    = ctx;
        this.biochemicalRenderer.renderDiagram(
            diagramConfig.key, 0, 0, width, height, diagramConfig.options
        );
    ...
    renderAllBiochemicalDiagrams() {
        return this.biochemicalDiagrams.map((_, index) => {
            try {
                return this.renderBiochemicalDiagramToPNG(index);
            } catch (error) {
    ...
    getAvailableBiochemicalDiagrams() {
        const diagrams   = {};
        const categories = BiochemicalDiagramsRegistry.getAllCategories();
        categories.forEach(category => {
            diagrams[category] = BiochemicalDiagramsRegistry.getDiagramsByCategory(category);
        });
        return diagrams;
    }

    listBiochemicalDiagrams() {
        return this.biochemicalDiagrams.map((diagram, index) => ({
            index,
            id:       diagram.id,
            title:    diagram.title,
            key:      diagram.key,
    ...
    removeBiochemicalDiagram(diagramIndex) {
        if (diagramIndex < 0 || diagramIndex >= this.biochemicalDiagrams.length) {
            throw new Error(`Biochemical diagram index ${diagramIndex} out of range`);
        }
        const removed = this.biochemicalDiagrams.splice(diagramIndex, 1);
        this.addToHistory(`Removed biochemical diagram: ${removed[0].title}`);
        return removed[0];
    }

    updateBiochemicalDiagram(diagramIndex, updates) {
        if (diagramIndex < 0 || diagramIndex >= this.biochemicalDiagrams.length) {
            throw new Error(`Biochemical diagram index ${diagramIndex} out of range`);
        }
        const diagram = this.biochemicalDiagrams[diagramIndex];
        if (updates.title)   diagram.title   = updates.title;
        if (updates.options) diagram.options = { ...diagram.options, ...updates.options };
        this.addToHistory(`Updated biochemical diagram: ${diagram.title}`);
        return diagram;
    }
...
exportToPNG(filename = 'spreadsheet.png', options = {}) {
        const {
            ...
            includeGraphs                  = false,
            includeBiochemicalDiagrams     = false,
            chartIndices           = [],
            anatomicalIndices      = [],
            chemistryIndices       = [],
            physicsIndices         = [],
        ...
        if (includeCharts && this.charts.length > 0) {
            visualizationsToRender.charts = chartIndices.length > 0
                ? chartIndices.map(i => this.charts[i]).filter(Boolean)
                : this.charts;
        }

        if (includeAnatomicalDiagrams && this.anatomicalDiagrams.length > 0) {
            visualizationsToRender.anatomical = anatomicalIndices.length > 0
        ...
        if (includeCrossSectionDiagrams && this.crossSectionDiagrams.length > 0) {
            visualizationsToRender.crossSection = crossSectionIndices.length > 0
        ...
        if (includeStereochemistryDiagrams && this.stereochemistryDiagrams.length > 0) {
            visualizationsToRender.stereochemistry = stereochemistryIndices.length > 0
        ...
        if (includeGeometricShapes && this.geometricShapes.length > 0) {
            visualizationsToRender.geometric = geometricIndices.length > 0
                ? geometricIndices.map(i => this.geometricShapes[i]).filter(Boolean)
                : this.geometricShapes;
        }

        if (includeGraphs && this.graphs.length > 0) {
            visualizationsToRender.graphs = graphIndices.length > 0
                ? graphIndices.map(i => this.graphs[i]).filter(Boolean)
                : this.graphs;
        }

        if (includeBiochemicalDiagrams && this.biochemicalDiagrams.length > 0) {
            visualizationsToRender.biochemical = biochemicalIndices.length > 0
                ? biochemicalIndices.map(i => this.biochemicalDiagrams[i]).filter(Boolean)
                : this.biochemicalDiagrams;
        }
        ...
        const totalVisualizations =
            ...
            visualizationsToRender.biochemical.length;
...
renderVisualizationsToCanvas(ctx, visualizations) {
        ...
        const allVisualizations = [
            ...
            ...biochemical.map(d    => ({ type: 'biochemical',     data: d, icon: '🧬' }))
        ];
        ...
        if (biochemical.length > 0)     summaryParts.push(`${biochemical.length} Biochemical`);
        ...
        const typeLabels = {
            ...
            'biochemical':     'Biochemical Diagram'
        };
        ...
        allVisualizations.forEach((viz, index) => {
            ...
            ctx.textAlign = 'left';
            ctx.fillText(`${viz.icon} ${index + 1}. ${viz.data.title}`, vizX, vizY - 15);
            ...
            try {
                ...
                if (viz.type === 'chart') {
                    ...
                    this.crossSectionRenderer.renderDiagram(
                        viz.data.key, 0, 0, itemWidth, itemHeight, viz.data.options
                    );
                    ...
                    this.geometricRenderer.renderShape(
                        viz.data.key, 0, 0, itemWidth, itemHeight, viz.data.config
                    );
                ...
                } else if (viz.type === 'biochemical') {
                    this.biochemicalRenderer.canvas = tempCanvas;
                    this.biochemicalRenderer.ctx    = tempCtx;
                    this.biochemicalRenderer.renderDiagram(
                        viz.data.key, 0, 0, itemWidth, itemHeight, viz.data.options
                    );
                }
...
async exportToExcel(filename = 'spreadsheet.xlsx', options = {}) {
        ...
        const hasVisualizations =
            ...
            (includeBiochemicalDiagrams     && this.biochemicalDiagrams.length > 0);
        ...
        if (hasVisualizations) {
            ...
            if (includeCharts && this.charts.length > 0) {
                ...
                chartHeaderCell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE7F3FF' } };
                ...
                for (let i = 0; i < this.charts.length; i++) {
                    ...
                    try {
                        ...
                        this.chartRenderer.renderChart(
                            chartCanvas, chartCtx,
                            chartConfig.key, chartConfig.data, chartConfig.options
                        );
            ...
            if (includeAnatomicalDiagrams && this.anatomicalDiagrams.length > 0) {
                ...
                anatomicalHeaderCell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFE7E7' } };
                vizSheet.getRow(currentRow).height = 30;
                currentRow += 2;
            ...
            if (includeChemistryDiagrams && this.chemistryDiagrams.length > 0) {
                ...
                chemistryHeaderCell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF3E0' } };
                vizSheet.getRow(currentRow).height = 30;
                currentRow += 2;
                ...
                for (let i = 0; i < this.chemistryDiagrams.length; i++) {
                    ...
                    try {
                        ...
                        await addImageToExcel(
                            ...
                            'chemistry', i,
                            { Category: diagramInfo.category, Formula: diagramInfo.formula || 'N/A' }
                        );
                    } catch (error) {
            ...
            if (includePhysicsDiagrams && this.physicsDiagrams.length > 0) {
                ...
                physicsHeaderCell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE3F2FD' } };
                vizSheet.getRow(currentRow).height = 30;
                currentRow += 2;
            ...
            if (includeCrossSectionDiagrams && this.crossSectionDiagrams.length > 0) {
                ...
                crossSectionHeaderCell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE7F9EF' } };
                vizSheet.getRow(currentRow).height = 30;
                currentRow += 2;
                ...
                for (let i = 0; i < this.crossSectionDiagrams.length; i++) {
                    ...
                    try {
                        ...
                        this.crossSectionRenderer.renderDiagram(
                            diagramConfig.key, 0, 0, diagramWidth, diagramHeight, diagramConfig.options
                        );
            ...
            if (includeStereochemistryDiagrams && this.stereochemistryDiagrams.length > 0) {
                ...
                stereochemHeaderCell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF4ECF7' } };
                vizSheet.getRow(currentRow).height = 30;
                currentRow += 2;
            ...
            if (includeGeometricShapes && this.geometricShapes.length > 0) {
                ...
                geometricHeaderCell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF3E0' } };
                vizSheet.getRow(currentRow).height = 30;
                currentRow += 2;
                ...
                for (let i = 0; i < this.geometricShapes.length; i++) {
                    ...
                    try {
                        ...
                        this.geometricRenderer.renderShape(
                            shapeConfig.key, 0, 0, shapeWidth, shapeHeight, shapeConfig.config
                        );
            ...
            if (includeGraphs && this.graphs.length > 0) {
                ...
                graphsHeaderCell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE3F2FD' } };
                vizSheet.getRow(currentRow).height = 30;
                currentRow += 2;
                ...
                for (let i = 0; i < this.graphs.length; i++) {
                    ...
                    try {
                        ...
                        const graphData = this.graphRenderer.renderGraph(
                            graphConfig.key, 0, 0, graphWidth, graphHeight, graphConfig.config
                        );
                        ...
                        if (graphData.rSquared !== undefined) metadata['R²']    = graphData.rSquared.toFixed(4);
                        if (graphData.slope    !== undefined) metadata['Slope'] = graphData.slope.toFixed(4);
            ...
            if (includeBiochemicalDiagrams && this.biochemicalDiagrams.length > 0) {
                console.log('\n🧬 Adding Biochemical Diagrams to Excel...');

                const biochemicalHeaderCell  = vizSheet.getCell(`A${currentRow}`);
                biochemicalHeaderCell.value  = '🧬 BIOCHEMICAL DIAGRAMS';
                biochemicalHeaderCell.font   = { bold: true, size: 14, color: { argb: 'FF00897B' } };
                biochemicalHeaderCell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE0F2F1' } };
                vizSheet.getRow(currentRow).height = 30;
                currentRow += 2;

                for (let i = 0; i < this.biochemicalDiagrams.length; i++) {
                    const diagramConfig = this.biochemicalDiagrams[i];
                    try {
                        const diagramWidth  = diagramConfig.options.width  || 720;
                        const diagramHeight = diagramConfig.options.height || 560;
                        const diagramCanvas = createCanvas(diagramWidth, diagramHeight);
                        const diagramCtx    = diagramCanvas.getContext('2d');

                        this.biochemicalRenderer.canvas = diagramCanvas;
                        this.biochemicalRenderer.ctx    = diagramCtx;
                        this.biochemicalRenderer.renderDiagram(
                            diagramConfig.key, 0, 0, diagramWidth, diagramHeight, diagramConfig.options
                        );

                        const diagramInfo = BiochemicalDiagramsRegistry.getDiagram(diagramConfig.key);
                        await addImageToExcel(
                            diagramCanvas,
                            `Biochemical ${i + 1}: ${diagramConfig.title}`,
                            'biochemical', i,
                            { Category: diagramInfo.category }
                        );
                    } catch (error) {
                        console.error(`  ❌ Biochemical Diagram ${i + 1} rendering failed:`, error.message);
                    }
                }
                currentRow += 2;
            }
        ...
        return {
            ...
            visualizations: {
                ...
                biochemicalDiagrams:      includeBiochemicalDiagrams     ? this.biochemicalDiagrams.length : 0,
                total:
                    (includeCharts                  ? this.charts.length : 0) +
                    (includeAnatomicalDiagrams      ? this.anatomicalDiagrams.length : 0) +
                    (includeChemistryDiagrams       ? this.chemistryDiagrams.length : 0) +
...
async exportSelectedVisualizations(baseFilename, options = {}) {
        ...
        const results = { png: null, excel: null };

        try {
            if (format === 'png' || format === 'both') {
                const pngFilename = `${baseFilename}.png`;
                this.exportToPNG(pngFilename, exportOptions);
                results.png = { success: true, filename: pngFilename };
            }
    ...
    async exportCompleteWorkbook(baseFilename = 'complete_workbook', format = 'both') {
        ...
        const exportOptions = {
            ...
            includeGraphs:                  true,
            includeBiochemicalDiagrams:     true
        };
        ...
        try {
            ...
            return {
                ...
                summary: {
                    ...
                    biochemicalDiagrams:      this.biochemicalDiagrams.length,
    ...
    async exportWithCharts(filename = 'workbook_with_charts') {
        return this.exportSelectedVisualizations(filename, { format: 'both', includeCharts: true });
    }

    async exportWithAnatomicalDiagrams(filename = 'workbook_with_biology') {
        return this.exportSelectedVisualizations(filename, { format: 'both', includeAnatomicalDiagrams: true });
    }

    async exportWithChemistryDiagrams(filename = 'workbook_with_chemistry') {
        return this.exportSelectedVisualizations(filename, { format: 'both', includeChemistryDiagrams: true });
    }

    async exportWithPhysicsDiagrams(filename = 'workbook_with_physics') {
        return this.exportSelectedVisualizations(filename, { format: 'both', includePhysicsDiagrams: true });
    }

    async exportWithCrossSectionDiagrams(filename = 'workbook_with_crosssection') {
        return this.exportSelectedVisualizations(filename, { format: 'both', includeCrossSectionDiagrams: true });
    }

    async exportWithStereochemistryDiagrams(filename = 'workbook_with_molecules') {
        return this.exportSelectedVisualizations(filename, { format: 'both', includeStereochemistryDiagrams: true });
    }

    async exportWithGeometricShapes(filename = 'workbook_with_geometric') {
        return this.exportSelectedVisualizations(filename, { format: 'both', includeGeometricShapes: true });
    }

    async exportWithGraphs(filename = 'workbook_with_graphs') {
        return this.exportSelectedVisualizations(filename, { format: 'both', includeGraphs: true });
    }

    async exportWithBiochemicalDiagrams(filename = 'workbook_with_biochemical') {
        return this.exportSelectedVisualizations(filename, { format: 'both', includeBiochemicalDiagrams: true });
    }
    ...
    getVisualizationSummary() {
        return {
            charts: {
                ...
                types: this.charts.map(c => ({ title: c.title, type: c.key }))
            },
            anatomicalDiagrams: {
                count:    this.anatomicalDiagrams.length,
                diagrams: this.anatomicalDiagrams.map(d => ({ title: d.title, category: d.category }))
            },
            chemistryDiagrams: {
                count:    this.chemistryDiagrams.length,
                diagrams: this.chemistryDiagrams.map(d => ({ title: d.title, category: d.category, formula: d.formula }))
            },
            physicsDiagrams: {
                count:    this.physicsDiagrams.length,
                diagrams: this.physicsDiagrams.map(d => ({ title: d.title, category: d.category }))
            },
            crossSectionDiagrams: {
                count:    this.crossSectionDiagrams.length,
                diagrams: this.crossSectionDiagrams.map(d => ({ title: d.title, category: d.category }))
            },
            stereochemistryDiagrams: {
                count:     this.stereochemistryDiagrams.length,
                molecules: this.stereochemistryDiagrams.map(d => ({ title: d.title, formula: d.formula }))
            },
            geometricShapes: {
                count:  this.geometricShapes.length,
                shapes: this.geometricShapes.map(d => ({ title: d.title, type: d.key, dimensionality: d.dimensionality }))
            },
            graphs: {
            ...
            biochemicalDiagrams: {
                count:    this.biochemicalDiagrams.length,
                diagrams: this.biochemicalDiagrams.map(d => ({ title: d.title, category: d.category }))
            },
            total:
                this.charts.length +
                ...
                this.graphs.length +
                this.biochemicalDiagrams.length
        };
    }
    ...
    hasAnyVisualizations() {
        return (
            ...
            this.biochemicalDiagrams.length > 0
        );
    }
    ...
    getVisualizationTypes() {
        ...
        if (this.biochemicalDiagrams.length > 0)    types.push('biochemical');
        return types;
    }
    ...
    getDiagramCounts() {
        return {
            ...
            graphs:          this.graphs.length,
            biochemical:     this.biochemicalDiagrams.length,
            total:
                this.charts.length +
                this.anatomicalDiagrams.length +
                this.chemistryDiagrams.length +
    ...
    async exportAllVisualizationsSeparately(folderPath) {
        ...
        for (let i = 0; i < this.charts.length; i++) {
            try {
                ...
                const canvas   = createCanvas(chart.options.width || 700, chart.options.height || 500);
                const ctx      = canvas.getContext('2d');
        ...
        for (let i = 0; i < this.anatomicalDiagrams.length; i++) {
            try {
                ...
                const canvas   = createCanvas(diagram.options.width || 800, diagram.options.height || 700);
                const ctx      = canvas.getContext('2d');
                this.diagramRenderer.canvas = canvas;
                this.diagramRenderer.ctx    = ctx;
        ...
        for (let i = 0; i < this.crossSectionDiagrams.length; i++) {
            try {
                ...
                const w        = diagram.options.width  || 800;
                const h        = diagram.options.height || 600;
                const canvas   = createCanvas(w, h);
                const ctx      = canvas.getContext('2d');
                this.crossSectionRenderer.canvas = canvas;
                this.crossSectionRenderer.ctx    = ctx;
                this.crossSectionRenderer.renderDiagram(diagram.key, 0, 0, w, h, diagram.options);
                fs.writeFileSync(filename, canvas.toBuffer('image/png'));
        ...
        for (let i = 0; i < this.stereochemistryDiagrams.length; i++) {
            try {
                ...
                this.stereochemistryRenderer.canvas = canvas;
                this.stereochemistryRenderer.ctx    = ctx;
                this.stereochemistryRenderer.renderDiagram(diagram.key, 0, 0, w, h, diagram.options);
                fs.writeFileSync(filename, canvas.toBuffer('image/png'));
        ...
        for (let i = 0; i < this.geometricShapes.length; i++) {
            try {
                ...
                const w        = shape.options.width  || 800;
                const h        = shape.options.height || 600;
                const canvas   = createCanvas(w, h);
                const ctx      = canvas.getContext('2d');
                this.geometricRenderer.canvas = canvas;
                this.geometricRenderer.ctx    = ctx;
                this.geometricRenderer.renderShape(shape.key, 0, 0, w, h, shape.config);
                fs.writeFileSync(filename, canvas.toBuffer('image/png'));
        ...
        for (let i = 0; i < this.graphs.length; i++) {
            try {
                ...
                this.graphRenderer.canvas = canvas;
                this.graphRenderer.ctx    = ctx;
                const graphData = this.graphRenderer.renderGraph(graph.key, 0, 0, 1000, 800, graph.config);
                fs.writeFileSync(filename, canvas.toBuffer('image/png'));
                results.graphs.push({
                    success:  true,
                    filename,
        ...
        for (let i = 0; i < this.biochemicalDiagrams.length; i++) {
            try {
                const diagram  = this.biochemicalDiagrams[i];
                const filename = `${folderPath}/biochemical_${i + 1}_${diagram.title.replace(/[^a-z0-9]/gi, '_')}.png`;
                const w        = diagram.options.width  || 720;
                const h        = diagram.options.height || 560;
                const canvas   = createCanvas(w, h);
                const ctx      = canvas.getContext('2d');
                this.biochemicalRenderer.canvas = canvas;
                this.biochemicalRenderer.ctx    = ctx;
                this.biochemicalRenderer.renderDiagram(diagram.key, 0, 0, w, h, diagram.options);
                fs.writeFileSync(filename, canvas.toBuffer('image/png'));
                results.biochemical.push({ success: true, filename, title: diagram.title });
                console.log(`✓ Biochemical ${i + 1}: ${diagram.title}`);
            } catch (error) {
                results.errors.push({ type: 'biochemical', index: i, error: error.message });
                console.error(`✗ Biochemical ${i + 1} failed: ${error.message}`);
            }
        }
        ...
        return {
            ...
            summary: {
                ...
                biochemicalExported:     results.biochemical.length,
                totalExported:
...
export { 
    ...
    BiochemicalDiagramsRegistry,
    AnatomicalShapes,
    BiochemicalDiagramRenderer
};
